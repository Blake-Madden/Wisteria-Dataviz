...
namespace Wisteria::GraphItems
    ...
    std::shared_ptr<Label> PieSlice::CreateMiddleLabel(wxDC& measureDC,
        ...
        pieLabel->GetGraphItemInfo().Pen(wxNullPen).
            Window(GetWindow()).Scaling(GetScaling()).
...
namespace Wisteria::Graphs
    ...
    void PieChart::SetData(std::shared_ptr<const Data::Dataset> data,
        ...
        for (const auto& group : outerGroups)
            ...
            GetOuterPie().emplace_back(
                SliceInfo{ m_groupColumn1->GetCategoryLabel(group.first),
                           group.second,
            ...
            for (const auto& innerGroupOuterRing : innerGroups)
                ...
                for (const auto& innerGroup : innerGroupOuterRing.second)
                    ...
                    currentOuterSliceSlices.emplace_back(
                        SliceInfo{ m_groupColumn2->GetCategoryLabel(innerGroup.first),
                               innerGroup.second,
                ...
                innerPie.insert(std::make_pair(
                    m_groupColumn1->GetCategoryLabel(innerGroupOuterRing.first),
    ...
    void PieChart::RecalcSizes()
        ...
        for (size_t i = 0; i < GetOuterPie().size(); ++i)
            ...
            const auto& middleLabel = pSlice->CreateMiddleLabel(measureDc, sliceProportion,
        ...
        for (size_t i = 0; i < GetInnerPie().size(); ++i)
            ...
            if (GetInnerPie().at(i).m_showText)
                ...
                connectionLine->SetWindow(GetWindow());
            ...
            const auto& middleLabel = pSlice->CreateMiddleLabel(measureDc,
