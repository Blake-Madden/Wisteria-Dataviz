      ...
      public:
        ...
        /** @brief Classifies a column of text values into previously defined categories
            ...
            @returns A pair of datasets.\n
                 \n
                 The first is a dataset filled with the comments and the categories that
                 ...
                 will appear multiple times in the output, with the different categories
                 next to it.\n
                 \n
                 The second is a dataset of all comments that were not classified.
                 This can be useful for retraining your classifier
                 (i.e., adding more regexes to your classifier dataset).
            @throws std::runtime_error If the file can't be read or named columns aren't found,
                throws an exception.\n
                The exception's @c what() message is UTF-8 encoded, so pass it to
                @c wxString::FromUTF8() when formatting it for an error message.*/
