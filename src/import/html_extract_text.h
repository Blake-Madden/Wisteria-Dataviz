...
namespace html_utilities
    ...
    public:
        ...
        [[nodiscard]] wchar_t find(const wchar_t letter) const;
    private:
    ...
    public:
        ...
        [[nodiscard]] wchar_t operator[](const wchar_t* html_entity) const
        ...
        [[nodiscard]] wchar_t find(const wchar_t* html_entity, const size_t length) const;
    private:
    ...
    */
    ...
    public:
        ...
        javascript_hyperlink_parse() noexcept :
          m_js_text_start(nullptr), m_js_text_end(nullptr), m_current_hyperlink_length(0)
            {}
        /** @brief Constructor.
        ...
        [[nodiscard]] const wchar_t* operator()() noexcept;
        /** @returns The length of the current hyperlink.*/
        [[nodiscard]] inline size_t get_current_hyperlink_length() const noexcept
    ...
    */
    ...
    public:
        ...
        [[nodiscard]] const wchar_t* get_base_url() const noexcept
            { return m_base; }
        /// @returns The length of the base web directory.
        [[nodiscard]] size_t get_base_url_length () const noexcept
        ...
        [[nodiscard]] inline bool is_current_link_an_image() const noexcept
            { return m_current_link_is_image; }
        /** @returns True if the current hyperlink is pointing to a javascript.*/
        [[nodiscard]] inline bool is_current_link_a_javascript() const noexcept
        ...
        [[nodiscard]] static const wchar_t* find_url_end(const wchar_t* text) noexcept
            {
        ...
        [[nodiscard]] constexpr static inline bool is_legal_url_character(const wchar_t ch)
            {
    ...
    public:
        ...
        [[nodiscard]] html_hyperlink_parse get_html_parser() const noexcept
            { return m_html_hyperlink_parse; }
        /** @returns The Javascript parser.*/
        [[nodiscard]] javascript_hyperlink_parse get_script_parser() const noexcept
            { return m_javascript_hyperlink_parse; }
        /** @returns The parsing method, either html or script.*/
        [[nodiscard]] hyperlink_parse_method get_parse_method() const noexcept
            { return m_method; }
        /** @returns the length of the current hyperlink.*/
        [[nodiscard]] inline size_t get_current_hyperlink_length() const noexcept
            {
    ...
    public:
        ...
        [[nodiscard]] std::wstring get_root_domain() const
        ...
        [[nodiscard]] static std::wstring parse_image_name_from_url(const wchar_t* url);
        /** @returns The top-level domain (e.g., .com or .org) from an url.
            @param url The url to analyze.*/
        [[nodiscard]] static std::wstring parse_top_level_domain_from_url(const wchar_t* url);
        /** @returns Whether an URL is just a domain and not a subfolder or file.
            @param url The url to analyze.*/
        [[nodiscard]] static bool is_url_top_level_domain(const wchar_t* url) noexcept;
    protected:
        ...
        [[nodiscard]] static size_t find_last_directory(std::wstring& url, size_t& query_postion);
    ...
    public:
        ...
        [[nodiscard]] const wchar_t* operator()(const wchar_t* html_text,
                                  const size_t text_length);
        };

    /// @returns @c true if character is not safe to use in an URL.
    /// @param character The character to review.
    [[nodiscard]] constexpr inline bool is_unsafe_uri_char(const wchar_t character)
        {
    ...
    public:
        /** @brief Main interface for extracting plain text from an HTML buffer.
            ...
            @param include_outer_text Whether text outside of the first and last <>
                should be included. Recommended true.
        ...
        [[nodiscard]] const std::wstring& get_title() const noexcept
            { return m_title; }
        /** @returns The subject from the metadata file or stream.
            @note Must be called after calling operator() or read_meta_data()
                (depending on which parser you are using).*/
        [[nodiscard]] const std::wstring& get_subject() const noexcept
            { return m_subject; }
        /** @returns The description from the metadata file or stream.
            @note Must be called after calling operator() or read_meta_data()
                (depending on which parser you are using).*/
        [[nodiscard]] const std::wstring& get_description() const noexcept
        ...
        /** @returns The keywords from the metadata file or stream.
            @note Must be called after calling operator() or read_meta_data()
                (depending on which parser you are using).*/
        [[nodiscard]] const std::wstring& get_keywords() const noexcept
            { return m_keywords; }
        /** Compares (case insensitively) raw HTML text with an element constant to see if the current element that
            ...
            text in between opening d closing tags.
            @returns True if the current position matches the element.
            @note Be sure to skip the starting '<' first.*/
        [[nodiscard]] static bool compare_element(const wchar_t* text, const wchar_t* element,
        ...
        [[nodiscard]] static bool compare_element_case_sensitive(const wchar_t* text, const wchar_t* element,
                                   const size_t element_size,
                                   const bool accept_self_terminating_elements = false);
        ...
        [[nodiscard]] static std::wstring get_element_name(const wchar_t* text,
                                             const bool accept_self_terminating_elements = true);
        /** @returns The matching > to a <, or null if not found.
            @param text The HTML stream to analyze.*/
        [[nodiscard]] static const wchar_t* find_close_tag(const wchar_t* text) noexcept;
        /** Reads the value inside of an element as a string.
        ...
        [[nodiscard]] static std::wstring read_element_as_string(const wchar_t* html_text,
        ...
        [[nodiscard]] static std::pair<const wchar_t*, size_t> read_attribute(const wchar_t* text,
            const wchar_t* tag, const size_t tagSize,
            const bool allowQuotedTags,
            const bool allowSpacesInValue = false);
        /** Same as read_attribute(), except it returns a standard string instead of a raw pointer.
            @param text The start of the element section.
        ...
        [[nodiscard]] static std::wstring read_attribute_as_string(const wchar_t* text,
            const wchar_t* attribute, const size_t attributeSize,
            const bool allowQuotedTags,
            const bool allowSpacesInValue = false);
        /** Same as read_attribute(), except it returns the value as a long.
            @param text The start of the element section.
        ...
        [[nodiscard]] static long read_attribute_as_long(const wchar_t* text,
            const wchar_t* attribute, const size_t attributeSize,
            const bool allowQuotedTags);
        ...
        [[nodiscard]] static const wchar_t* find_element(const wchar_t* sectionStart,
        ...
        /** Searches a buffer range for an element's matching close (e.g., "</h1>").
            ...
            @param elementTagLength The length of elementTag.*/
        [[nodiscard]] static const wchar_t* find_closing_element(const wchar_t* sectionStart,
                                           const wchar_t* sectionEnd,
                                           const wchar_t* elementTag,
                                           const size_t elementTagLength);
        /** @brief Searches for an attribute inside of an element.
            ...
            @note If the position is currently on a '<' and you are searching for its matching '>', then you should
                  set @c sectionStart to one character after the '<'.*/
        [[nodiscard]] static const wchar_t* find_tag(const wchar_t* text,
            const wchar_t* tag, const size_t tagSize,
            const bool allowQuotedTags);
        ...
        [[nodiscard]] static const std::pair<const wchar_t*,std::wstring> find_bookmark(const wchar_t* sectionStart,
                                             const wchar_t* sectionEnd);
        ...
        [[nodiscard]] static const wchar_t* strchr_not_quoted(const wchar_t* string, const wchar_t ch) noexcept;
        /** Searches for substring in string (case-insensitive), but ensures that
            ...
            @returns The pointer of where the character is, or null if not found.*/
        [[nodiscard]] static const wchar_t* stristr_not_quoted(
        ...
        [[nodiscard]] static std::string parse_charset(const char* pageContent, const size_t length);
    protected:
        ...
        [[nodiscard]] static std::wstring convert_symbol_font_section(const std::wstring& symbolFontText);
