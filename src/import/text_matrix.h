...
namespace lily_of_the_valley
    ...
    class text_matrix
      ...
      public:
        ...
        explicit text_matrix(std::vector<std::vector<string_typeT>>* matrix) : m_matrix(matrix) {}
        ...
        explicit text_matrix(std::vector<string_typeT>* vector) : m_vector(vector) {}
        ...
        void add_row_definition(const text_row<string_typeT>& row) { m_rows.push_back(row); }

        /// @brief Clears the row definitions from the parser.
        void remove_rows() noexcept { m_rows.clear(); }

        /** @brief The main function which reads a block of text and divides it up
                into columns & rows and returns the number of rows read.
            @details The row count must be correct (it won't read any more than what you
                specify here). The column count must be the widest row. Different rows may
                have a different number of columns, but you should specify the widest
                possible amount of columns to avoid a jagged array.
        ...
        size_t read(const wchar_t* text, const size_t row_count, const size_t column_count,
            ...
            for (auto pos = m_rows.begin(); pos != m_rows.end(); ++pos)
                {
                // some row definitions may be used more than once for consecutive rows
                for (size_t i = 0;
                     (!pos->get_repeat_count() ? true : i < pos->get_repeat_count().value()); ++i)
                    {
                    ...
                    // if ignoring blank lines, then let this row parser go until it reads in one
                    // valid row
                    if (ignore_blank_lines)
                        {
                        /* If ignoring blank lines then use the current row reader's iteration until
                           it actually reads in a row of text. If we are using a row reader for 5
                           rows and we have 2 rows of text, a blank row, and three more rows of text
                           then we read 2, skip the blank, and then read the next three. If we are
                           NOT ignoring blank rows, then we would just read the first five rows and
                           treat the blank one as one of the iterations of the current row reader.*/
                        for (;;)
                            {
                            const wchar_t* originalPosition = currentPosition;
                            currentPosition = pos->read(currentPosition);
                            // if any columns were read then stop with the current iteration of this
                            // row reader
                            ...
                            // if no columns were read then check to see if we are at the
                            // end of the file
                            ...
                            /* Otherwise, we are at the end of the line, so move to the start of the
                               next one. Skip the line feed if following a carriage return-- this is
                               the standard for Windows files.*/
                            if (currentPosition > text && currentPosition[-1] == 13 &&
                                currentPosition[0] == 10)
                                {
                                ++currentPosition;
                                }
                            /* if this row reader's columns are all set to skip the columns of text
                               then that is why no columns were read, so we should break here. Of
                               course, make sure the read line really wasn't blank by checking if
                               the start of the line was an EOL.*/
                        ...
                        if (pos->is_reading_text())
                            {
                            // Finished reading all columns for this row. Now, if this row has a
                            ...
                            if (m_matrix)
                                ...
                                if (pos->is_column_resizing_enabled() &&
                                    pos->get_number_of_columns_last_read() <
                                        m_matrix->at(currentRowIndex).size())
                                    {
                                    m_matrix->at(currentRowIndex)
                                        .resize(pos->get_number_of_columns_last_read());
                                    }
                                }
                            ++currentRowIndex;
                            }
