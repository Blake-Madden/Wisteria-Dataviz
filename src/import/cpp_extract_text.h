...
namespace lily_of_the_valley
    {
    /** @brief Class to extract text (Doxygen/Javadoc comments and GETTEXT resources) from a C++ source code stream.*/
    ...
    protected:
        ...
        /// @brief Adds the text to the buffer, but strips markup (e.g., Doxygen tags) from the stream first.
    ...
    private:
        ...
        std::set<std::wstring> m_doxygen_tags =
            { L"{", L"}", L"a", L"b", L"c", L"e", L"p", L"em", L"brief", L"short", L"code", L"endcode",
              L"note", L"return", L"returns", L"result", L"remark", L"remarks", L"retval",
              L"warning", L"sa", L"see", L"related", L"relates", L"relatedalso", L"relatesalso" };
        // doxygen tags that are removed (but their following text is read in)
        ...
        std::set<std::wstring> m_doxygen_tags_single_line =
            { L"class", L"struct", L"union", L"var",
              L"enum", L"def", L"typedef", L"file",
              L"namespace", L"package", L"interface",
              L"fn", L"name", L"addtogroup" };
        };
    }
