...
namespace lily_of_the_valley
    ...
    /// @private
    enum class JUST{ justL, justR, justC, justF };
    ...
    /// @private
    enum class SBK{ sbkNon, sbkCol, sbkEvn, sbkOdd, sbkPg };
    /// @private
    enum class PGN{ pgDec, pgURom, pgLRom, pgULtr, pgLLtr };
    ...
    /// @private
    enum class RDS{ rdsNorm, rdsSkip };

    // Rtf Internal State
    /// @private
    enum class RIS{ risNorm, risBin, risHex };
    ...
    /// @private
    enum class IPROP{ ipropBold, ipropItalic, ipropUnderline, ipropStrikeThrough, ipropLeftInd,
                      ...
                      ipropMax };
    /// @private
    enum class ACTN{ actnSpec, actnByte, actnWord };
    /// @private
    enum class PROPTYPE{ propChp, propPap, propSep, propDop };
    ...
    /// @private
    enum class IPFN{ ipfnBin, ipfnHex, ipfnSkipDest };
    /// @private
    enum class IDEST{ idestPict, idestSkip };
    /// @private
    ...
    struct rtf_symbol
        {
        rtf_symbol(const char* keyword, const int dfault,
                   const bool passDflt, const KWD command,
                   const int index, const wchar_t* stringToPrint) :
            szKeyword(keyword), dflt(dfault), fPassDflt(passDflt),
            kwd(command), idx(index), printString(stringToPrint)
            {}
        rtf_symbol() = delete;
        [[nodiscard]]
    ...
    public:
        ...
        std::set<rtf_symbol>::const_iterator find(const char* keyword) const
            ...
            return m_symbols.find(rtf_symbol(keyword,
                /*the rest of this is filler*/0, false, KWD::kwdChar, 0, L""));
            }
    protected:
    ...
    /** @brief Class to extract text from an RTF stream.
        ...
        @par References
        Adapted from the MSDN example code from http://msdn.microsoft.com/en-us/library/aa140300#rtfspec_60.
    ...
    public:
        ...
        explicit rtf_extract_text(const rtf_extraction_type& extraction_type =
                                  rtf_extraction_type::rtf_to_text) noexcept;
        ...
        void set_style_prefix(const std::wstring& prefix)
            { m_style_prefix = prefix; }
