...
bool lily_of_the_valley::markdown_extract_text::parse_styled_text(wchar_t& previousChar,
    ...
    while (m_currentStart < m_currentEndSentinel && *m_currentStart == tag)
        {
        std::advance(m_currentStart, 1);
        }
    const std::wstring_view currentTag{ tagStart, static_cast<size_t>(
                                                      std::distance(tagStart, m_currentStart)) };
    ...
    while ((endOfTag < m_currentEndSentinel) && *endOfTag == tag)
        {
        std::advance(endOfTag, 1);
        }
...
bool lily_of_the_valley::markdown_extract_text::parse_html_block(const std::wstring_view tag,
    ...
    const auto* endOfTag = string_util::find_matching_close_tag(
        { m_currentStart,
...
lily_of_the_valley::markdown_extract_text::operator()(const std::wstring_view md_text)
    ...
    while (m_currentStart != nullptr && (m_currentStart < m_currentEndSentinel) &&
           (*m_currentStart != 0))
        {
        if (*m_currentStart == L'\\')
            {
            ...
            if (!isEscaping)
                ...
                if (std::wcsncmp(m_currentStart, BEGIN_FIGURE.data(), BEGIN_FIGURE.length()) == 0)
                    {
                    m_currentStart += BEGIN_FIGURE.length();
                    continue;
                    }
                if (std::wcsncmp(m_currentStart, END_FIGURE.data(), END_FIGURE.length()) == 0)
                    {
                    m_currentStart += END_FIGURE.length();
                    continue;
                    }
                if (std::wcsncmp(m_currentStart, L"\\@ref(", 6) == 0)
                    {
                    m_currentStart += 6;
                    const auto* endOfTag = string_util::find_unescaped_matching_close_tag_same_line(
                        m_currentStart, L'(', L')');
                ...
                if (std::wcsncmp(m_currentStart, L"\\newpage", 8) == 0)
                    {
                    m_currentStart += 8;
                    add_characters(L"\n\n");
                    continue;
        ...
        // skip over header tags
            ...
            if (!isEscaping && (previousChar == L'\n' || previousChar == L'\r'))
                {
                while ((m_currentStart < m_currentEndSentinel) && *m_currentStart == L'#')
                    {
                    ++m_currentStart;
                    }
        ...
        // code blocks
            ...
            if (!isEscaping && std::wcsncmp(m_currentStart, L"```", 3) == 0)
                ...
                const auto* scanAhead{ m_currentStart };
                ...
                if (const auto* const includeFalse =
                        std::wcsstr(m_currentStart, L"#| include: false");
                    includeFalse != nullptr && includeFalse < endOfTag)
                    {
                    m_currentStart = endOfTag + 3;
                    }
                else if (const auto* const echoFalse =
            ...
            if (!isEscaping)
                ...
                if (std::wcsncmp(m_currentStart, L"`r drop_cap(", 11) == 0)
                    {
                    m_currentStart += 11;
                    if (m_currentStart + 1 < m_currentEndSentinel &&
                        (m_currentStart[1] == L'\'' || m_currentStart[1] == L'"'))
                ...
                if (std::wcsncmp(m_currentStart, L"`r menu(", 8) == 0)
                    {
                    m_currentStart += 8;
        ...
        // links
            ...
            if (!isEscaping)
                {
                const auto* labelStart{ ++m_currentStart };
                const auto* endOfTag = string_util::find_unescaped_matching_close_tag_same_line_n(
                    m_currentStart, L'[', L']',
                    std::distance(m_currentStart, m_currentEndSentinel));
                if (endOfTag == nullptr || (endOfTag >= m_currentEndSentinel))
                ...
                if (*m_currentStart == L'(')
                    {
                    const auto* labelEnd{ m_currentStart - 1 };
                    ++m_currentStart;
                    endOfTag = string_util::find_unescaped_matching_close_tag_same_line_n(
                        m_currentStart, L'(', L')',
                        std::distance(m_currentStart, m_currentEndSentinel));
        ...
        // RMarkdown (Pandoc) comment
            ...
            {
            const auto* const endOfTag = std::wcsstr(m_currentStart, L"-->");
            if (endOfTag == nullptr)
                {
        ...
        // table
            ...
            {
            previousChar = L'|';
            const auto* scanAhead{ std::next(m_currentStart) };
