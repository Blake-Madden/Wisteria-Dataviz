...
namespace Wisteria::GraphItems
    ...
    public:
        ...
        ReferenceLine(const AxisType axisType, const double axisPosition,
                      ...
                      {}
        /// @private
        ///  Lines are the same if the line color and label are the same.
        ///  If those are different, then compares on the starting point on the parent axis.
        [[nodiscard]] bool operator<(const ReferenceLine& that) const noexcept
            {
            return (m_compKey.CmpNoCase(that.m_compKey) < 0) ?
                true :
                (m_axisPosition < that.m_axisPosition);
            }
    private:
    ...
    public:
        ...
        struct TickMark
            ...
            DisplayType m_displayType{ DisplayType::NoDisplay }; /*!< @brief The display of the tickmark.*/
            double m_position{ 0 };                              /*!< @brief Value on the axis.*/
            wxCoord m_lineLength{ 0 };                           /*!< @brief Length of the line, protruding from the axis.*/
            wxCoord m_physicalCoordinate{ -1 };                  /*!< @brief Actual pixel coordinate on the graph.*/
            };
        ...
        public:
            ...
            /** @brief Constructor.
                @param pos1 The starting position (relative to its parent axis) of where to place the bracket.
            ...
            [[nodiscard]] double GetStartPosition() const noexcept
            ...
            /** @brief Indicates whether the bracket is only pointing to one spot on the axis.
                @returns `true` if the bracket is really just a single line pointing from a bracket label to the axis.
            ...
            /// @returns The width of the padding between the bracket label and the parent axis (not including the connection line).
            /// @warning This will need to be scaled when being drawn or measured. (This is normally managed by the parent axis.)
            [[nodiscard]] wxCoord GetPadding() const noexcept
                { return m_padding; }
            /// @brief Sets the width of the padding between the bracket label and the parent axis (not including the connection line).
            /// @param padding The width of the padding. This is a pixel value that the framework will scale to the screen for you.
            ///  (The parent axis will also scale this as the graph's scaling changes.)
            void SetPadding(const wxCoord padding) noexcept
                { m_padding = padding; }
            ...
            [[nodiscard]] Label& GetLabel() noexcept
                { return m_label; }

            /// @returns The label position (relative to the parent axis).
            ...
            /// @returns The length of the tickmarks between the main line of the bracket and the parent axis.
            /// @warning This will need to be scaled when being drawn or measured.
            ///  (This is normally managed by the parent axis.)
            [[nodiscard]] wxCoord GetTickmarkLength() const noexcept
                { return m_tickMarkLength; }
            /// @brief Sets the length of the connection line between the bracket label and the parent axis.
            /// @param length The length of the tickmarks. This is a pixel value that the framework will scale to the screen for you.
            ///  (The parent axis will also scale this as the graph's scaling changes.)
            ...
            /// @private
            [[nodiscard]] const Label& GetLabel() const noexcept
                { return m_label; }
            /// @private
            [[nodiscard]] const wxPen& GetLinePen() const noexcept
                { return m_linePen; }
        private:
            ...
            [[nodiscard]] const Orientation& GetOrientation() const noexcept
        ...
        public:
            ...
            [[nodiscard]] bool IsShown() const noexcept
                { return m_show; }
            /// @param display Sets whether to display the axis label.
            void Show(const bool display) noexcept
                { m_show = display; }
            /// @brief Sets the value on the parent axis at the specified value.
            /// @param value The value to set the axis point to.
            void SetValue(const double value) noexcept
                { m_value = value; }
            /// @returns The point's value along the parent axis.
        ...
        private:
            ...
            [[nodiscard]] double GetPhysicalCoordinate() const noexcept
        ...
        explicit Axis(const AxisType type) :
            ...
            m_labelOrientation((type == AxisType::BottomXAxis || type == AxisType::TopXAxis) ?
                AxisLabelOrientation::Parallel : AxisLabelOrientation::Perpendicular),
            // label stacking looks a bit odd on vertical axes, so don't enable auto stack for them
        ...
        void SetRange(double rangeStart, double rangeEnd,
                      uint8_t precision, double interval,
        ...
        /** @brief Adds a point at value/label to the axis.
            @details This function should only be called if you need to fill the axis with
             ...
             This is useful if you want uneven values such as 1, 5, 15, 50, 100 to be shown across an axis equidistantly.

             This behaves differently from custom labels in that the custom labels are merely cosmetic displays;
             this will actually set the position on axis to the value. In the above example, plotting a value of 75
             will place the point in the middle of where 50 and 100 are.
            ...
            @warning Call AdjustRangeToLabels() after you are finished adding your axis values to adjust the range.
        ...
        void StartAtZero(const bool startAtZero) noexcept
            ...
            SetRange(axisMin, axisMax, GetPrecision(), GetInterval(), GetDisplayInterval());
            }
        /// @returns The interval size between ticks on the axis.
        [[nodiscard]] double GetInterval() const noexcept
            { return m_interval; }
        ...
        /** @name Label & Value Functions
            ...
            @details The axis labels are managed by default labels generated by SetRange() or AddUnevenAxisPoint(),
             ...
             Finally, the axis's title, header, footer, and brackets manage their own font attributes. To change these,
             edit them via GetHeader(), GetFooter(), GetTitle(), and GetBrackets().*/
        /// @{

        /// @returns The number of axis points where the major tickmarks and labels will appear.
        /// @note This, combined with GetDisplayInterval(), can be useful for filling the axis with custom labels.
        [[nodiscard]] size_t GetAxisPointsCount() const noexcept
            { return m_axisLabels.size(); }
        /// @returns The major axis points (generated by SetRange()).
        [[nodiscard]] std::vector<AxisPoint>& GetAxisPoints() noexcept
            {
        ...
        [[nodiscard]] const AxisLabelAlignment& GetPerpendicularLabelAxisAlignment() const noexcept
            { return m_axisLabelAlignment; }
        ...
        [[nodiscard]] RelativeAlignment GetParallelLabelAlignment() const noexcept
        ...
        /** @brief Sets whether the first and last axis labels should be shown (the outer lines of the plot).
        ...
        /** @returns Whether the first and last axis labels are being shown (the outer lines of the plot).*/
        ...
        [[nodiscard]] const AxisLabelOrientation& GetAxisLabelOrientation() const noexcept
        ...
        /** @brief Gets/sets the pen used for the main axis line and tickmarks.
            ...
            @note Set to `wxNullPen` or transparent to turn off the main axis line and tickmarks.*/
        [[nodiscard]] wxPen& GetAxisLinePen() noexcept
            { return m_axisLinePen; }

        /** @brief Gets/sets the gridline pen.
            @returns The gridline pen.
            @note Set to `wxNullPen` or transparent to turn off gridlines.*/
        [[nodiscard]] wxPen& GetGridlinePen() noexcept
            { return m_gridlinePen; }

        /** @brief Returns how the end of the axis line is being drawn.
            @returns The line's cap style.*/
        [[nodiscard]] AxisCapStyle GetCapStyle() const noexcept
            { return m_capStyle; }
        /** @brief Sets how the end of the axis line is being drawn.
            @param capStyle The cap style to use.*/
        void SetCapStyle(const AxisCapStyle capStyle) noexcept
            { m_capStyle = capStyle; }
        /// @}
        ...
        [[nodiscard]] double GetTickMarkInterval() const noexcept
        ...
        [[nodiscard]] const TickMark::DisplayType& GetTickMarkDisplay() const noexcept
        ...
        /** @brief Gets/sets the footer of the axis. This is a label that appears at the bottom or left of the axis
             (depending on the orientation of the axis).

             For X axes, relative alignment controls where the footer is aligned vertically against the left of the axis
             (the default is to align it centered to the axis line).
        ...
        [[nodiscard]] double GetOffsetFromParentAxis() const noexcept
        ...
        /// @returns The amount of pixels (scaled to the system's DPI and canvas's scale) that the largest
        // tick mark is pushing out from the axis. This is useful for see how wide the axis line and its tickmarks are.
        [[nodiscard]] wxCoord CalcTickMarkOuterWidth() const;
        /// @returns The amount of pixels (scaled to the system's DPI and canvas's scale) that the largest
        /// tick mark is pushing in from the axis.
        [[nodiscard]] wxCoord CalcTickMarkInnerWidth() const;
        /// @}

        // These are overridden so that the widest and tallest labels are invalidated.
        /// @private
        [[nodiscard]] GraphItemInfo& GetGraphItemInfo() noexcept final
            {
            m_widestLabel = m_tallestLabel = Label(GraphItemInfo().Ok(false));
        ...
        /// @private
        [[nodiscard]] wxFont& GetFont() noexcept final
            {
            m_widestLabel = m_tallestLabel = Label(GraphItemInfo().Ok(false));
            return GraphItemBase::GetFont();
            }
        /// @private
        [[nodiscard]] const wxFont& GetFont() const noexcept final
            { return GraphItemBase::GetFont(); }
        /// @private
        ...
        /// @private
        [[nodiscard]] const wxPen& GetAxisLinePen() const noexcept
            { return m_axisLinePen; }
        /// @private
        [[nodiscard]] const wxPen& GetGridlinePen() const noexcept
            { return m_gridlinePen; }
        /// @private
        [[nodiscard]] const Label& GetFooter() const noexcept
            { return m_footer; }
        /// @private
        [[nodiscard]] const Label& GetHeader() const noexcept
            { return m_header; }
        /// @private
        [[nodiscard]] const Label& GetTitle() const noexcept
            { return m_title; }
        /// @private
        [[nodiscard]] const std::vector<AxisBracket>& GetBrackets() const noexcept
            { return m_brackets; }
        /// @private
    ...
    private:
        ...
        [[nodiscard]] wxRect GetContentRect() const noexcept final
            ...
            if (IsVertical())
                ...
                lineRect.SetWidth(ScaleToScreenAndCanvas(GetAxisLinePen().GetWidth()));
                return lineRect;
                }
            else
                {
                auto lineRect = wxRect(GetLeftPoint(), GetRightPoint());
                lineRect.SetHeight(ScaleToScreenAndCanvas(GetAxisLinePen().GetWidth()));
                return lineRect;
                }
            }
        ...
        [[nodiscard]] double GetLabelPhysicalOffset() const noexcept
        ...
        [[nodiscard]] std::vector<TickMark>& GetCustomTickMarks() noexcept
            { return m_customTickMarks; }

        /// @returns The calculated tick marks.
        /// @sa GetCustomTickMarks().
        [[nodiscard]] std::vector<TickMark>& GetTickMarks() noexcept
            { return m_tickMarks; }
        /// @returns The (const) calculated tick marks.
        /// @sa GetCustomTickMarks().
        [[nodiscard]] const std::vector<TickMark>& GetTickMarks() const noexcept
            { return m_tickMarks; }
        ...
        wxDateTime m_fyQ1{ wxDateTime(1, wxDateTime::Month::Jul, 1970) }; // year is just a placeholder here
