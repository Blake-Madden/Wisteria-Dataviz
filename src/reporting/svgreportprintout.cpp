        ...
        if (options.m_includeLayoutOptions || hasLayerControls)
            ...
            if (options.m_includeLayoutOptions)
                ...
                header += wxString::Format(
                    ...
                    "    const leavingStacked = layout === 2;\n"
                    "    const enteringStacked = !leavingStacked && (layout + 1) %% 3 === 2;\n"
                    "    layout = (layout + 1) %% 3;\n"
                    "    if (leavingStacked) {\n"
                    ...
                    "    } else if (enteringStacked) {\n"
                    "      // reverse the paint order now, while pages are still at their\n"
                    "      // pre-stacked positions, so the deferred applyLayout() call below\n"
                    "      // has a painted frame to transition the transform from\n"
                    "      const ps = document.getElementById('pageset');\n"
                    "      const vis = getVisiblePages();\n"
                    "      for (let i = vis.length - 1; i >= 0; --i) ps.appendChild(vis[i]);\n"
                    ...
                    "    }\n"
                    "    if (leavingStacked || enteringStacked) {\n"
  ...
  window.addEventListener('load', tagInk);
        ...
        if (options.m_includeSlideshow)
            ...
            header +=
                ...
                "    const page = pages[i];\n"
                "    const xfrm = (page.style.transform && page.style.transform !== 'none' ? "
                "page.style.transform : page.getAttribute('transform')) || '';\n"
                "    const match = "
                "xfrm.match(/translate\\(\\s*[\\d.\\-]+\\s*(?:px)?\\s*,\\s*([\\d.\\-]+)\\s*(?:px)?"
                "\\s*\\)/);\n"
