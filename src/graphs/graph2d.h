...
#ifndef WISTERIA_GRAPH2D_H
#define WISTERIA_GRAPH2D_H
    ...
    class Graph2D : public GraphItems::GraphItemBase
      ...
      public:
        ...
        /** @brief Sets/gets the graph's title.
            @note The title's relative alignment controls where the title is aligned against the
                plot (i.e., whether it is center, right aligned, etc.).\n
        ...
        /** @brief Sets/gets the graph's subtitle.
            @note The subtitle's relative alignment controls where the subtitle is aligned against
                the plot.\n
                Its display info controls its font, color, and other formatting settings.
        ...
        /** @brief Sets/gets the graph's caption.
            @note The caption's relative alignment controls where the caption is aligned against
                the plot.\n
                Its display info controls its font, color, and other formatting settings.
            @returns The caption.*/
        [[nodiscard]]
        ...
        void SetStippleShape(const Icons::IconShape shape) noexcept { m_stippleShape = shape; }
        ...
        /** @brief Sets the background image being drawn within the plotting area
                (inside the main axes).
        ...
        void AddProperty(const wxString& key, const wxVariant& val) { m_properties[key] = val; }
        ...
        [[nodiscard]]
        const std::shared_ptr<Icons::Schemes::IconScheme>& GetShapeScheme() const noexcept
            {
      ...
      protected:
        ...
        GetContinuousColumnRequired(const wxString& colName) const
            {
            auto continuousCol = GetDataset()->GetContinuousColumn(colName);
        ...
        /** @brief Sets the rectangular area of the entire graph area.
            @param rect The rectangle to bound the entire plot to. This is relative to its parent
           canvas.
            @param dc Measurement DC, which is not used in this implementation.
