...
wxIMPLEMENT_DYNAMIC_CLASS(Wisteria::Graphs::Histogram, Wisteria::Graphs::BarChart)
        ...
        for (const auto& blockTable : groups.get_data())
            ...
            if (foundBar == GetBars().end())
                ...
                if (blockTable.second.first.size() > 1)
                    ...
                    theBar.GetBlocks()[0].GetSelectionLabel().SetLabelStyle(
                        LabelStyle::DottedLinedPaperWithMargins);
                    }
                AddBar(theBar);
                }
            else
                ...
                if (blockTable.second.first.size() > 1)
                    {
                    block.GetSelectionLabel().SetLabelStyle(
                        LabelStyle::DottedLinedPaperWithMargins);
                    block.GetSelectionLabel().GetHeaderInfo().Enable(true).LabelAlignment(
            ...
            if (!IsShowingFullRangeOfValues())
                {
                GetBarAxis().SetCustomLabel(
                    barNumber++, GraphItems::Label(wxNumberFormatter::ToString(
                                     blockTable.first.m_bin,
                                     (has_fractional_part(blockTable.first.m_bin)) ? 2 : 0,
                                     Settings::GetDefaultNumberFormat())));
                }
    ...
    void Histogram::SortIntoRanges(const std::optional<size_t> binCount)
        ...
        const size_t numOfBins = binCount.has_value() ?
                                     binCount.value() :
        ...
        if (GetBinningMethod() == BinningMethod::BinByIntegerRange)
            ...
            if (GetRoundingMethod() != RoundingMethod::NoRounding && isLowestValueBeingAdjusted)
                {
                minVal -= 1;
                }
        ...
        std::vector<std::vector<comparable_first_pair<Data::GroupIdType, valuesCounter>>> bins(
            numOfBins);
        for (size_t i = 0; i < GetDataset()->GetRowCount(); ++i)
            {
            if (std::isnan(m_continuousColumn->GetValue(i)))
                ...
                if (compare_doubles(currentVal, minVal))
                    ...
                    if (foundGroup != bins[0].end())
                        ...
                        if (foundGroup->second.second.size() < Settings::GetMaxObservationInBin())
                            {
                            foundGroup->second.second.emplace(
        ...
        // if the starting point or interval size has floating precision then
        // set the axis to show it
        if (GetBinningMethod() != BinningMethod::BinByIntegerRange &&
        ...
        for (size_t i = 0; i < bins.size(); ++i)
            ...
            for (const auto& block : bins[i])
                ...
                // Note that this will be zero if grouping is not in use.
                const size_t colorIndex =
                    IsUsingGrouping() ? GetSchemeIndexFromGroupId(block.first) : 0;

                const wxColour blockColor =
                    GetColorScheme() ? (IsUsingGrouping() ? GetColorScheme()->GetColor(colorIndex) :
                                                            GetColorScheme()->GetColor(0)) :
                                       wxTransparentColour;
                const wxBrush blockBrush =
                    (IsUsingGrouping() ? GetBrushScheme()->GetBrush(colorIndex) :
                                         GetBrushScheme()->GetBrush(0));
                ...
                if (block.second.second.size() > 1)
                    ...
                    theBlock.GetSelectionLabel().SetLabelStyle(
                        LabelStyle::DottedLinedPaperWithMargins);
                    theBlock.GetSelectionLabel().GetHeaderInfo().Enable(true).LabelAlignment(
                        TextAlignment::Centered);
                    }
                ...
                (GetBinLabelDisplay() == BinLabelDisplay::BinPercentage) ?
                    wxString::Format(
                        ...
                        wxNumberFormatter::ToString(
                            percentage, 0, wxNumberFormatter::Style::Style_NoTrailingZeroes)) :
                    wxString::Format(
