        ...
        const auto drawBox = [&](auto& box, const bool measureOnly, const size_t boxIndex)
        {
            ...
            if (box.GetDataset()->GetRowCount() > 1)
                ...
                AddObject(std::make_unique<GraphItems::Polygon>(
                    GraphItems::GraphItemInfo{ whiskerLabel }.Pen(linePen).Brush(lineBrush).Scaling(
                        GetScaling()),
                    linePoints));
                ...
                if (IsBoxEffectStylized(box.GetBoxEffect()))
                    ...
                    if (box.GetBoxEffect() == BoxEffect::Pencil)
                        ...
                        auto whiskerLines =
                            std::make_unique<GraphItems::Lines>(pencilPen, GetScaling());
                        whiskerLines->SetLineStyle(LineStyle::Pencil);
                        // vertical stem
                        whiskerLines->AddLine(box.m_upperOutlierRangeCoordinate,
                                              box.m_lowerOutlierRangeCoordinate);
                        // lower cap
                        whiskerLines->AddLine(
                        ...
                        whiskerLines->AddLine(
                            ...
                            wxPoint(box.m_upperOutlierRangeCoordinate.x + (boxWidth / 4),
                                    box.m_upperOutlierRangeCoordinate.y));
                        AddObject(std::move(whiskerLines));
                        }
                    else
                        {
                        const auto whiskerWidth = ScaleToScreenAndCanvas(2);
                        const bool isWaterColor =
                        ...
                        const auto drawWobblyLine = [&](wxPoint from, wxPoint to)
                        {
                            ...
                            for (int segment = 1; segment < SEGMENTS; ++segment)
                                ...
                                pts.emplace_back(
                                    static_cast<int>(
                                        from.x + dx * segment +
                                        wobbleDist(GraphItems::ShapeRenderer::GetRNG())),
                                    static_cast<int>(
                                        from.y + dy * segment +
                                        wobbleDist(GraphItems::ShapeRenderer::GetRNG())));
                                }
                ...
                // color-filled box
                    ...
                    if (box.GetBoxEffect() == BoxEffect::Pencil)
                        ...
                        const auto axisPenWidth = GetLeftYAxis().GetAxisLinePen().IsOk() ?
                                                      GetLeftYAxis().GetAxisLinePen().GetWidth() :
                                                      1;
                        boxPoly->GetPen().SetWidth(axisPenWidth * 1.5);
                        }
            ...
            for (size_t i = 0; i < box.GetDataset()->GetRowCount(); ++i)
                ...
                if (GetPhysicalCoordinates(box.GetXAxisPosition(), continuousColumn->GetValue(i),
                    ...
                    if (continuousColumn->GetValue(i) > box.GetUpperWhisker() ||
                        continuousColumn->GetValue(i) < box.GetLowerWhisker())
                        {
                        outliers->AddPoint(GraphItems::Point2D(
                                               GraphItems::GraphItemInfo{
                        ...
                        {
                        dataPoints->AddPoint(GraphItems::Point2D(
                                                 GraphItems::GraphItemInfo{
                                                     box.GetDataset()->GetIdColumn().GetValue(i) }
                                                     .AnchorPoint(pt)
                                                     .Brush(GetPointColor())
