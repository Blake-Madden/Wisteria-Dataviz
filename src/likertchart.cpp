...
namespace Wisteria::Graphs
    ...
    void LikertChart::SetLabels(std::shared_ptr<Data::Dataset>& data,
        ...
        {
        for (auto& catColumnName : categoricalColumns)
            {
    ...
    LikertChart::LikertSurveyQuestionFormat LikertChart::Simplify(
        ...
        if (currentFormat == LikertSurveyQuestionFormat::SevenPoint ||
            ...
            {
            Collapse7PointsTo3(data, categoricalColumns, CreateLabels(LikertSurveyQuestionFormat::ThreePoint));
        ...
        // 6 -> 2
            ...
            {
            Collapse6PointsTo2(data, categoricalColumns, CreateLabels(LikertSurveyQuestionFormat::TwoPoint));
        ...
        // 5 -> 3
            ...
            {
            Collapse5PointsTo3(data, categoricalColumns, CreateLabels(LikertSurveyQuestionFormat::ThreePoint));
            return (IsCategorized(currentFormat) ? LikertSurveyQuestionFormat::ThreePointCategorized : LikertSurveyQuestionFormat::ThreePoint);
            }
        // 4 -> 2
            ...
            {
            Collapse4PointsTo2(data, categoricalColumns, CreateLabels(LikertSurveyQuestionFormat::TwoPoint));
            return (IsCategorized(currentFormat) ? LikertSurveyQuestionFormat::TwoPointCategorized : LikertSurveyQuestionFormat::TwoPoint);
            }
        ...
        // just ensure their labels are correct
            ...
            {
            SetLabels(data, categoricalColumns, CreateLabels(LikertSurveyQuestionFormat::ThreePoint));
            }
        else if (currentFormat == LikertSurveyQuestionFormat::TwoPoint ||
            currentFormat == LikertSurveyQuestionFormat::TwoPointCategorized)
            {
            SetLabels(data, categoricalColumns, CreateLabels(LikertSurveyQuestionFormat::TwoPoint));
            }
    ...
    void LikertChart::Collapse4PointsTo2(std::shared_ptr<Data::Dataset>& data,
        ...
        for (const auto& catColumnName : categoricalColumns)
            {
            auto categoricalColumn = data->GetCategoricalColumn(catColumnName);
            if (categoricalColumn == data->GetCategoricalColumns().end())
                {
    ...
    void LikertChart::SetData(std::shared_ptr<const Data::Dataset> data,
        ...
        m_neutralLabel.clear();

        const auto groupColumn = (groupColumnName ? data->GetCategoricalColumn(groupColumnName.value()) :
            data->GetCategoricalColumns().cend());
        ...
        for (auto questionIter = categoricalColumns.crbegin();
             questionIter != categoricalColumns.crend();
             ++questionIter)
            {
            ...
            if (GetSurveyType() == LikertSurveyQuestionFormat::TwoPoint ||
                GetSurveyType() == LikertSurveyQuestionFormat::TwoPointCategorized)
                {
                SetNegativeLabel(categoricalColumn->GetCategoryLabel(1), 1);
                SetPositiveLabel(categoricalColumn->GetCategoryLabel(2), 1);
                }
            else if (GetSurveyType() == LikertSurveyQuestionFormat::ThreePoint ||
                GetSurveyType() == LikertSurveyQuestionFormat::ThreePointCategorized)
                {
                SetNegativeLabel(categoricalColumn->GetCategoryLabel(1), 1);
                SetNeutralLabel(categoricalColumn->GetCategoryLabel(2));
                SetPositiveLabel(categoricalColumn->GetCategoryLabel(3), 1);
                }
            else if (GetSurveyType() == LikertSurveyQuestionFormat::FourPoint ||
                GetSurveyType() == LikertSurveyQuestionFormat::FourPointCategorized)
                {
                SetNegativeLabel(categoricalColumn->GetCategoryLabel(1), 1);
                SetNegativeLabel(categoricalColumn->GetCategoryLabel(2), 2);
                SetPositiveLabel(categoricalColumn->GetCategoryLabel(3), 1);
                SetPositiveLabel(categoricalColumn->GetCategoryLabel(4), 2);
                }
                ...
                SetNegativeLabel(categoricalColumn->GetCategoryLabel(1), 1);
                SetNegativeLabel(categoricalColumn->GetCategoryLabel(2), 2);
                SetNeutralLabel(categoricalColumn->GetCategoryLabel(3));
                SetPositiveLabel(categoricalColumn->GetCategoryLabel(4), 1);
                SetPositiveLabel(categoricalColumn->GetCategoryLabel(5), 2);
                }
            else if (GetSurveyType() == LikertSurveyQuestionFormat::SixPoint ||
                GetSurveyType() == LikertSurveyQuestionFormat::SixPointCategorized)
                {
                SetNegativeLabel(categoricalColumn->GetCategoryLabel(1), 1);
                SetNegativeLabel(categoricalColumn->GetCategoryLabel(2), 2);
                SetNegativeLabel(categoricalColumn->GetCategoryLabel(3), 3);
                SetPositiveLabel(categoricalColumn->GetCategoryLabel(4), 1);
                SetPositiveLabel(categoricalColumn->GetCategoryLabel(5), 2);
                SetPositiveLabel(categoricalColumn->GetCategoryLabel(6), 3);
                }
                ...
                SetNegativeLabel(categoricalColumn->GetCategoryLabel(2), 2);
                SetNegativeLabel(categoricalColumn->GetCategoryLabel(3), 3);
                SetNeutralLabel(categoricalColumn->GetCategoryLabel(4));
                SetPositiveLabel(categoricalColumn->GetCategoryLabel(5), 1);
                SetPositiveLabel(categoricalColumn->GetCategoryLabel(6), 2);
                SetPositiveLabel(categoricalColumn->GetCategoryLabel(7), 3);
                }
            }
        }

    //----------------------------------------------------------------
    void LikertChart::AddSurveyQuestion(const wxString& question, const Data::ColumnWithStringTable& groups, const Data::ColumnWithStringTable& responses)
        {
        if (responses.GetRowCount() == 0)
            { return; }
        wxASSERT_LEVEL_2_MSG(IsCategorized(), L"Categorized data being passed into non-categorized Likert chart!");
        ...
        if (GetSurveyType() == LikertSurveyQuestionFormat::TwoPointCategorized)
            ...
            for (const auto& group : fMap)
                {
                const LikertThreePointSurveyQuestion surveyQuestion(groups.GetCategoryLabel(group.first),
            ...
            for (const auto& group : fMap)
                {
                const LikertFivePointSurveyQuestion surveyQuestion(groups.GetCategoryLabel(group.first),
            ...
            for (const auto& group : fMap)
                {
                const LikertSevenPointSurveyQuestion surveyQuestion(groups.GetCategoryLabel(group.first),
                    findCount(group.first, 1), findCount(group.first, 2), findCount(group.first, 3),
    ...
    //----------------------------------------------------------------
    void LikertChart::AddSurveyQuestion(const wxString& question, const Data::ColumnWithStringTable& responses)
        {
        if (responses.GetRowCount() == 0)
            { return; }
        wxASSERT_LEVEL_2_MSG(!IsCategorized(), L"Non-categorized data being loaded into categorized Likert chart!");
    ...
    void LikertChart::AddSurveyQuestion(const LikertThreePointSurveyQuestion& question)
        ...
        m_negativeBlockSize = std::max(m_negativeBlockSize, next_interval(question.m_negativeRate, 2));
        m_positiveBlockSize = std::max(m_positiveBlockSize, next_interval(question.m_positiveRate, 2));
    ...
    void LikertChart::AddSurveyQuestion(const LikertCategorizedThreePointSurveyQuestion& question)
        ...
        for (const auto& category : question.m_threePointCategories)
            {
            m_negativeBlockSize = std::max(m_negativeBlockSize, next_interval(category.m_negativeRate, 2));
            m_positiveBlockSize = std::max(m_positiveBlockSize, next_interval(category.m_positiveRate, 2));
            m_neutralBlockSize = std::max(m_neutralBlockSize, next_interval(category.m_neutralRate, 2));
    ...
    void LikertChart::AddSurveyQuestion(const LikertFivePointSurveyQuestion& question)
        ...
        m_negativeBlockSize = std::max(m_negativeBlockSize, next_interval(question.m_negative1Rate+question.m_negative2Rate, 2));
        m_positiveBlockSize = std::max(m_positiveBlockSize, next_interval(question.m_positive1Rate+question.m_positive2Rate, 2));
        m_neutralBlockSize = std::max(m_neutralBlockSize, next_interval(question.m_neutralRate, 2));
        m_naBlockSize = std::max(m_naBlockSize, next_interval(question.m_naRate, 2));
        // to see if we even need a neutral or NA sections later
        m_neutralMaxSize = std::max(m_neutralMaxSize, question.m_neutralRate);
    ...
    void LikertChart::AddSurveyQuestion(const LikertCategorizedFivePointSurveyQuestion& question)
        ...
        for (const auto& category : question.m_fivePointCategories)
            {
            m_negativeBlockSize = std::max(m_negativeBlockSize, next_interval(category.m_negative1Rate + category.m_negative2Rate, 2));
            m_positiveBlockSize = std::max(m_positiveBlockSize, next_interval(category.m_positive1Rate + category.m_positive2Rate, 2));
            m_neutralBlockSize = std::max(m_neutralBlockSize, next_interval(category.m_neutralRate, 2));
            m_naBlockSize = std::max(m_naBlockSize, next_interval(category.m_naRate, 2));
            // to see if we even need a neutral or NA sections later
            m_neutralMaxSize = std::max(m_neutralMaxSize, category.m_neutralRate);
    ...
    void LikertChart::AddSurveyQuestion(const LikertSevenPointSurveyQuestion& question)
        ...
        m_negativeBlockSize = std::max(m_negativeBlockSize, next_interval(question.m_negative1Rate+question.m_negative2Rate+question.m_negative3Rate, 2));
        m_positiveBlockSize = std::max(m_positiveBlockSize, next_interval(question.m_positive1Rate+question.m_positive2Rate+question.m_positive3Rate, 2));
        m_neutralBlockSize = std::max(m_neutralBlockSize, next_interval(question.m_neutralRate, 2));
        m_naBlockSize = std::max(m_naBlockSize, next_interval(question.m_naRate, 2));
        // to see if we even need a neutral or NA sections later
        m_neutralMaxSize = std::max(m_neutralMaxSize, question.m_neutralRate);
    ...
    void LikertChart::AddSurveyQuestion(const LikertCategorizedSevenPointSurveyQuestion& question)
        ...
        for (const auto& category : question.m_sevenPointCategories)
            {
            m_negativeBlockSize = std::max(m_negativeBlockSize, next_interval(category.m_negative1Rate+category.m_negative2Rate+category.m_negative3Rate, 2));
            m_positiveBlockSize = std::max(m_positiveBlockSize, next_interval(category.m_positive1Rate+category.m_positive2Rate+category.m_positive3Rate, 2));
            m_neutralBlockSize = std::max(m_neutralBlockSize, next_interval(category.m_neutralRate, 2));
            m_naBlockSize = std::max(m_naBlockSize, next_interval(category.m_naRate, 2));
            // to see if we even need a neutral or NA sections later
            m_neutralMaxSize = std::max(m_neutralMaxSize, category.m_neutralRate);
    ...
    void LikertChart::AddSurveyQuestionBar(const LikertThreePointSurveyQuestion& question)
        ...
        Bar currentBar(GetBarSlotCount()+1,
            { // the question
              BarBlock(BarBlockInfo(IsShowingResponseCounts() ? m_questionBlockSize-m_responseCountBlockSize : m_questionBlockSize).
        ...
        for (auto category = question.m_threePointCategories.crbegin();
            ...
            Bar currentBar(GetBarSlotCount()+1,
                { // empty space for parent question
                  ...
                  // the category
                  BarBlock(BarBlockInfo(IsShowingResponseCounts() ? m_categoryBlockSize-m_responseCountBlockSize : m_categoryBlockSize).
                    Brush(wxTransparentColor).SelectionLabel(Label(GraphItemInfo().Pen(*wxTRANSPARENT_PEN))).
                    Decal(
            ...
            { // the question
              ...
              BarBlock(BarBlockInfo(question.m_positive2Rate).
                ...
                Decal(Label(GraphItemInfo((IsShowingPercentages() && question.m_positive2Rate > 0) ? wxNumberFormatter::ToString(question.m_positive2Rate, 0, wxNumberFormatter::Style::Style_NoTrailingZeroes)+L"%" : wxString()).
                      ...
                      ChildAlignment(RelativeAlignment::FlushLeft))) ),
              // block after positive
              BarBlock(BarBlockInfo(m_positiveBlockSize-(question.m_positive1Rate+question.m_positive2Rate)).Show(false)),
              // neutral block
              BarBlock(BarBlockInfo(question.m_neutralRate).
                Brush(GetNeutralColor()).SelectionLabel(Label(GraphItemInfo().Pen(*wxTRANSPARENT_PEN))).
                Decal(Label(GraphItemInfo((IsShowingPercentages() && question.m_neutralRate > 0) ? wxNumberFormatter::ToString(question.m_neutralRate, 0, wxNumberFormatter::Style::Style_NoTrailingZeroes)+L"%" : wxString()).
    ...
    void LikertChart::AddSurveyQuestionBar(const LikertCategorizedSevenPointSurveyQuestion& question)
        ...
        Bar questionBar(safe_divide<double>(question.m_sevenPointCategories.size(),2)+GetBarSlotCount()+0.5f,
            { // main question
              BarBlock(BarBlockInfo(IsShowingResponseCounts() ? m_questionBlockSize-m_responseCountBlockSize : m_questionBlockSize).
                Brush(wxTransparentColor).SelectionLabel(Label(GraphItemInfo().Pen(*wxTRANSPARENT_PEN))).
                Decal(Label(GraphItemInfo(question.m_question).
    ...
    void LikertChart::RecalcSizes()
        ...
        if (m_neutralMaxSize > 0)
            ...
            neutralDividerLine.SetCustomXPosition(m_questionBlockSize+m_categoryBlockSize+m_negativeBlockSize+m_positiveBlockSize);
            ...
            for (auto& bar : GetBars())
                ...
                for (;;)
                    {
                    const auto foundPos = std::find_if(bar.GetBlocks().cbegin(), bar.GetBlocks().cend(),
        ...
        if (m_naMaxSize > 0)
            ...
            naDividerLine.SetCustomXPosition(m_questionBlockSize+m_categoryBlockSize+m_negativeBlockSize+m_positiveBlockSize+m_neutralBlockSize);
        ...
        // if showing categorized data, then only draw full horizontal divider lines between main questions
        ...
        auto sectionDividerLines = std::make_shared<GraphItems::Lines>(wxPen(*wxBLACK, 1, wxPENSTYLE_LONG_DASH), GetScaling());
        wxCoord bottomPosAndNegX{0}, bottomNeutralX{0}, bottomNAX{0};
        if (GetBottomXAxis().GetPhysicalCoordinate(m_questionBlockSize + m_categoryBlockSize + m_negativeBlockSize, bottomPosAndNegX))
            {
        ...
        if (GetBottomXAxis().GetPhysicalCoordinate(m_questionBlockSize + m_categoryBlockSize + m_negativeBlockSize + m_positiveBlockSize, bottomNeutralX))
            {
        ...
        if (GetBottomXAxis().GetPhysicalCoordinate(m_questionBlockSize + m_categoryBlockSize + m_negativeBlockSize + m_positiveBlockSize + m_neutralBlockSize, bottomNAX))
            {
