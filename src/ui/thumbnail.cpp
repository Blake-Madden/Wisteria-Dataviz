...
namespace Wisteria::UI
    {
    //----------------------------------
    EnlargedImageWindow::EnlargedImageWindow(const wxBitmap& bitmap, wxWindow* parent, wxWindowID id /*= wxID_ANY*/,
                          const wxPoint& pos /*= wxDefaultPosition*/, const wxSize& size /*= wxDefaultSize*/,
                          long style /*= wxFRAME_NO_TASKBAR|wxSTAY_ON_TOP|wxFULL_REPAINT_ON_RESIZE*/) :
        wxDialog(parent, id, wxEmptyString, pos, size, style, L"EnlargedImageWindow"), m_bitmap(bitmap)
        {
    ...
    //----------------------------------
    bool DropThumbnailImageFile::OnDropFiles([[maybe_unused]] wxCoord x, [[maybe_unused]] wxCoord y, const wxArrayString &filenames)
        {
    ...
    //----------------------------------
    Thumbnail::Thumbnail(wxWindow* parent, const wxBitmap& bmp, const ClickMode clickMode /*= FullSizeViewable*/, const bool allowFileDrop /*= false*/, wxWindowID id /*= wxID_ANY*/, const wxPoint& pos /*= wxDefaultPosition*/,
                         const wxSize& size /*= wxDefaultSize*/, long style /*= wxFULL_REPAINT_ON_RESIZE|wxBORDER_SIMPLE*/, const wxString& name /*= L"Thumbnail"*/) :
        ...
        // if original image is smaller or same size as this control, then no reason to have "click to view" support
    ...
    void Thumbnail::OnClick([[maybe_unused]] wxMouseEvent& event)
        ...
        if ((m_clickMode == ClickMode::FullSizeViewable) && m_img.IsOk())
            ...
            if (wxSize(scaledSize.first, scaledSize.second) != canvasBmp.GetSize())
                { canvasBmp = wxBitmap(canvasBmp.ConvertToImage().Rescale(scaledSize.first, scaledSize.second, wxIMAGE_QUALITY_HIGH)); }
            ...
            memDC.DrawRoundedRectangle(canvasBmp.GetWidth()-(width+14), canvasBmp.GetHeight()-(height+14),
                width+8, height+8, 2);
            memDC.DrawText(label, canvasBmp.GetWidth()-(width+10), canvasBmp.GetHeight()-(height+10));
            ...
            memDC.SetPen(*wxBLACK_PEN);
                ...
                memDC.DrawLine(0,memDC.GetSize().GetHeight()-1,memDC.GetSize().GetWidth(),memDC.GetSize().GetHeight()-1);
                memDC.DrawLine(0,0,0,memDC.GetSize().GetHeight());
                memDC.DrawLine(memDC.GetSize().GetWidth()-1,0,memDC.GetSize().GetWidth()-1,memDC.GetSize().GetHeight());
            ...
            {
            wxFileDialog fileDlg(this, _(L"Select an Image"), wxEmptyString, wxEmptyString,
