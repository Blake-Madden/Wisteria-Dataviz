...
namespace Wisteria::UI
    ...
    wxIMPLEMENT_CLASS(CodeEditor, wxStyledTextCtrl)
        ...
        MarkerDefine(wxSTC_MARKNUM_FOLDEREND, wxSTC_MARK_DOTDOTDOT, markerColor,
                     markerContrastColor);
    ...
    void CodeEditor::SetThemeColor(const wxColour& background)
        ...
        if (wxSTC_LEX_LUA == m_lexer)
            ...
            StyleSetForeground(wxSTC_C_COMMENTDOCKEYWORDERROR,
                               contrast.Contrast(m_commentDocColor));
            }
    ...
    void CodeEditor::SetLanguage(const int lang)
        ...
        if (wxSTC_LEX_LUA == m_lexer)
            ...
            SetKeyWords(
                0,
                _DT(L"alignas alignof and_eq asm atomic_cancel atomic_commit atomic_noexcept auto "
        ...
        if (event.GetKey() == GetLibraryAccessor())
            ...
            // or an object
                ...
                if (objectName == L"()")
                    ...
                    const wxString libraryNameAndFuncSig =
                        GetTextRange(objectNameStart, wordStart - 3);
