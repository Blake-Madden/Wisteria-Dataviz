...
namespace Wisteria::UI
    ...
    SideBar::SideBar(wxWindow* parent, wxWindowID id /*= wxID_ANY*/)
        : wxScrolledCanvas(parent, id, wxDefaultPosition, wxDefaultSize,
                           wxWANTS_CHARS | wxVSCROLL | wxBORDER_THEME | wxCLIP_CHILDREN |
    ...
    void SideBar::DrawGlassEffect(wxDC& dc, const wxRect rect, const wxColour& color)
        ...
        dc.GradientFillLinear(
            wxRect(rect.GetX(), rect.GetY(), rect.GetWidth(), rect.GetHeight() * .25),
            color.ChangeLightness(115), color.ChangeLightness(155), wxSOUTH);
        }
    ...
    void SideBar::ResetState()
        ...
        for (auto& item : m_folders)
            ...
            if (statePos != m_stateInfo.cend())
                ...
                if ((!item.m_subItems.empty() && !item.m_selectedItem.has_value()) ||
    ...
    void SideBar::OnChar(wxKeyEvent& event)
        ...
        if (event.GetKeyCode() == WXK_DOWN)
            {
            if (IsFolderSelected() &&
                (m_folders[GetSelectedFolder().value()].GetSubItemCount() != 0U) &&
        ...
        // if going up, select the previous item. If it has subitems, then select the last subitem.
            ...
            if (IsFolderSelected() &&
                ...
                SelectSubItem(GetSelectedFolder().value() - 1,
                              (m_folders[GetSelectedFolder().value() - 1].GetSubItemCount() != 0U) ?
                                  m_folders[GetSelectedFolder().value() - 1].GetSubItemCount() - 1 :
                                  0);
                }
            }
    ...
    SideBar::GetSelectedSubItemId() const
        {
        ...
        return std::make_pair(
            m_folders[GetSelectedFolder().value()].m_id,
            m_folders[GetSelectedFolder().value()]
                .m_subItems[m_folders[GetSelectedFolder().value()].m_selectedItem.value()]
                .m_id);
        }
    ...
    wxString SideBar::GetSelectedLabel() const
        ...
        if (m_folders[GetSelectedFolder().value()].IsSubItemSelected())
            {
    ...
    //---------------------------------------------------
    std::pair<std::optional<size_t>, std::optional<size_t>>
    SideBar::FindSubItem(const wxWindowID parentId, const wxWindowID subItemId) const
        {
    ...
    //---------------------------------------------------
    std::pair<std::optional<size_t>, std::optional<size_t>>
    SideBar::FindSubItem(const wxWindowID parentId, const SideBarSubItem& subItem) const
        {
        const auto parentIndex = FindFolder(parentId);
        if (!parentIndex.has_value())
            {
    ...
    size_t SideBar::GetFolderWidth(const size_t item)
        ...
        parentWidth += IsValidImageId(m_folders[item].m_iconIndex) ?
                           GetIconSize().GetWidth() + wxSizerFlags::GetDefaultBorder() :
                           0;
    ...
    //---------------------------------------------------
    void SideBar::InsertItem(const size_t position, const wxString& label, const wxWindowID Id,
    ...
    void SideBar::OnDraw(wxDC& dc)
        ...
        const wxDCFontChanger fc(dc, wxSystemSettings::GetFont(wxSYS_DEFAULT_GUI_FONT));
        ...
        const auto drawFolderBackground = [this, &dc](const size_t folderIndex)
        {
            ...
            if (GetSelectedFolder().has_value() && GetSelectedFolder().value() == folderIndex &&
                ...
                if (GetStyle() == SidebarStyle::Glassy)
                    ...
                    {
                    const wxDCBrushChanger bc(dc, m_selectedColor);
                    const wxDCPenChanger pc(dc, m_selectedColor);
                    dc.DrawRectangle(wxRect(buttonBorderRect.GetLeftTop().x,
                                            buttonBorderRect.GetLeftTop().y, GetSize().GetWidth(),
                                            GetItemHeight()));
                    }
                }
            else if (m_highlightedFolder.has_value() && m_highlightedFolder.value() == folderIndex)
                {
                if (GetStyle() == SidebarStyle::Glassy)
                    {
                    DrawGlassEffect(dc,
                    ...
                    {
                    const wxDCBrushChanger bc(dc, m_highlightColor);
                    const wxDCPenChanger pc(dc, m_highlightColor);
                    dc.DrawRectangle(wxRect(buttonBorderRect.GetLeftTop().x,
                                            buttonBorderRect.GetLeftTop().y, GetSize().GetWidth(),
                                            GetItemHeight()));
                    }
        ...
        for (size_t i = firstFolderToDraw.value(); i <= lastFolderToDraw.value(); ++i)
            {
                ...
                if (IsValidImageId(m_folders[i].m_iconIndex))
                    ...
                    {
                    dc.DrawLabel(
                        m_folders[i].m_label, wxNullBitmap,
                        wxRect(m_folders[i].m_Rect).Deflate(wxSizerFlags::GetDefaultBorder()),
                        wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL);
                    }
                }
            // draw subitems (if folder is expanded)
            if (m_folders[i].m_isExpanded && !m_folders[i].m_subItems.empty())
                {
                ...
                for (size_t j = startSubitem; j <= endSubitem; ++j)
                    ...
                    if (m_folders[i].m_highlightedItem.has_value() &&
                        m_folders[i].m_highlightedItem.value() == j && !subitemIsSelected)
                        {
                        ...
                        if (GetStyle() == SidebarStyle::Glassy)
                            ...
                            dc.DrawRectangle(wxRect(
                                buttonBorderRect.GetLeftTop().x, buttonBorderRect.GetLeftTop().y,
                                GetSize().GetWidth() >= GetSubitemIndentation() ?
                                    GetSize().GetWidth() - GetSubitemIndentation() :
                                    0,
                                GetItemHeight()));
                        ...
                        if (IsValidImageId(m_folders[i].m_subItems[j].m_iconIndex))
                            ...
                            {
                            dc.DrawLabel(m_folders[i].m_subItems[j].m_label, wxNullBitmap,
                                         wxRect(m_folders[i].m_subItems[j].m_Rect)
                                             .Deflate(wxSizerFlags::GetDefaultBorder()),
                                         wxALIGN_LEFT | wxALIGN_CENTER_VERTICAL);
                            }
    ...
    void SideBar::OnMouseChange(const wxMouseEvent& event)
        ...
        for (size_t i = 0; i < m_folders.size(); ++i)
            ...
            if (m_folders[i].m_isExpanded)
                ...
                for (size_t j = 0; j < m_folders[i].m_subItems.size(); ++j)
                    {
                    if (m_folders[i].m_subItems[j].m_Rect.Contains(event.GetX() + x,
                                                                   event.GetY() + y))
                        {
                        ...
                        // then we need to repaint it.
                        if (previouslyHighlightedFolder.has_value() &&
                            previouslyHighlightedFolder.value() == i && m_folders[i].IsActive() &&
        ...
        wxRect refreshRect =
            ...
            (m_highlightedRect && !m_highlightedIsSelected) ? m_highlightedRect.value() :
                                                              wxRect{};
    ...
    void SideBar::Realize()
        ...
        for (const auto& bmp : GetImageList())
            {
            if (bmp.IsOk() && GetIconSize().GetHeight() + GetPaddingHeight() > GetItemHeight())
                {
    ...
    std::pair<size_t, size_t> SideBar::CalculateItemRects()
        ...
        for (auto& folder : m_folders)
            {
            folder.m_Rect = wxRect(0, (GetItemHeight() * currentFolderIndex) + subItemHeight,
            ...
            for (size_t subCounter = 0; subCounter < folder.m_subItems.size(); ++subCounter)
                ...
                if (folder.m_isExpanded)
                    {
                    folder.m_subItems[subCounter].m_Rect = wxRect(
                        0, (GetItemHeight() * (currentFolderIndex + 1)) + subItemHeight,
                        GetClientSize().GetWidth() - GetSubitemIndentation(), GetItemHeight());
                    ...
                    if (!foundSelectedItem)
                        ...
                        // if this is the selected subitem, then we included it and we are done with
                        // that area
                        if (folder.IsSubItemSelected() &&
                            folder.m_selectedItem.value() == subCounter)
                            {
            ...
            // and it isn't expanded or has no subitems, then we are done getting the hit of the
        ...
        return std::make_pair((GetItemHeight() * m_folders.size()) + subItemHeight,
                              heightToSelectedItem);
        }
    ...
    void SideBar::EnsureFolderVisible(const size_t index)
        ...
        const wxRect lastItemRectToBeVisible =
            (m_folders[index].m_isExpanded && (m_folders[index].GetSubItemCount() != 0U) &&
        ...
        if (!scrolledClientRect.Contains(lastItemRectToBeVisible) && yUnit != 0)
            ...
            Scroll(wxDefaultCoord,
                   (lastItemRectToBeVisible.GetY() + (lastItemRectToBeVisible.GetHeight() / 2)) /
                       yUnit);
            }
        }
    ...
    bool SideBar::SelectFolder(const size_t item, const bool setFocus /*= true*/,
        ...
        if (m_folders[item].GetSubItemCount() != 0U)
            {
    ...
    bool SideBar::SelectSubItemById(const wxWindowID folderId, const wxWindowID subItemId,
        ...
        wxRect refreshRect = (needsExpanding ? GetClientRect() :
    ...
    bool SideBar::SelectSubItem(const size_t item, const size_t subItem,
        ...
        // if bogus subitem, then just select the parent folder
        if (m_folders[item].GetSubItemCount() == 0 || subItem >= m_folders[item].GetSubItemCount())
            {
