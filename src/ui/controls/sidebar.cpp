...
void SideBar::DrawGlassEffect(wxDC& dc, const wxRect rect, const wxColour& color)
    ...
    dc.GradientFillLinear(wxRect(rect.GetX(), rect.GetY(), rect.GetWidth(), rect.GetHeight() * .25),
        ...
        if (m_folders[i].m_isExpanded && !m_folders[i].m_subItems.empty())
            ...
            for (size_t j = startSubitem; j <= endSubitem; ++j)
                ...
                if (m_folders[i].m_highlightedItem.has_value() &&
                    ...
                    {
                    const wxDCTextColourChanger tcc(
                        dc, (m_folders[i].m_highlightedItem.has_value() &&
    ...
    for (auto& folder : m_folders)
        ...
        for (size_t subCounter = 0; subCounter < folder.m_subItems.size(); ++subCounter)
            ...
            if (folder.m_isExpanded)
                {
                folder.m_subItems[subCounter].m_Rect =
                    wxRect(0, (GetItemHeight() * (currentFolderIndex + 1)) + subItemHeight,
...
bool SideBar::SelectSubItemById(const wxWindowID folderId, const wxWindowID subItemId,
    ...
    wxRect refreshRect =
        (needsExpanding ? GetClientRect() : previouslySelectedRect.Union(currentlySelectedRect));
    refreshRect.Offset(0, -std::min(y, refreshRect.y));
    ...
    if (sendEvent)
        ...
        event.SetInt(m_folders[GetSelectedFolder().value()].m_subItems[subItemIndex.value()].m_id);
        event.SetEventObject(this);
        GetEventHandler()->ProcessEvent(event);
        }
    return true;
...
//-------------------------------------------
bool SideBar::SelectSubItem(const size_t item, const size_t subItem, const bool setFocus /*= true*/,
                            const bool sendEvent /*= true*/)
    {
    if (item >= GetFolderCount())
        {
