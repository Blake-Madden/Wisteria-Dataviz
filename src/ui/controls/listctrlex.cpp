...
ListEditSpinCtrlDouble::ListEditSpinCtrlDouble(wxWindow* parent, ListCtrlEx* owner,
                       ...
                       const wxString& name /*= L"ListEditSpinCtrlDouble"*/) :
                            wxSpinCtrlDouble(parent, id, value, pos, size, style, Min, Max,
                                             initial, 1.0, name),
                            m_owner(owner), m_editedRow(wxNOT_FOUND),
                            m_editedColumn(wxNOT_FOUND)
    {
    SetDigits(1);
...
void ListEditSpinCtrlDouble::Accept()
    ...
    if (m_editedRow != wxNOT_FOUND && m_editedColumn != wxNOT_FOUND &&
        ...
        m_owner->SetItemText(m_editedRow, m_editedColumn, std::isnan(GetValue()) ?
            wxString{} : wxString::Format(L"%0.1f", GetValue()));
        m_owner->Refresh();
        m_owner->SetItemBeenEditedByUser(true);
        }
    }
...
public:
            ...
            if (dc)
                ...
                const auto drawTables = [this, page, marginX, &bodyStart]
                    ...
                    for (const auto currentPageTableRowStart : currentPage.m_rowStarts)
                        ...
                        for (auto i = currentPageTableRowStart;
                            ...
                            if (i <= GetLastRow() && !m_list->GetAlternateRowColour().IsOk())
                                ...
                                const wxItemAttr rowAttributes =
                                    (m_list->IsVirtual() && virtualAttrib) ? *virtualAttrib :
                                    wxItemAttr(m_list->GetItemTextColour(i), m_list->GetItemBackgroundColour(i),
                        ...
                        for (auto columnCounter = 0;
                             columnCounter < m_list->GetColumnCount();
                             ++columnCounter)
                            {
                        ...
                        for (auto i = currentPageTableRowStart;
                            ...
                            for (auto j = 0; j < m_list->GetColumnCount(); ++j)
                                ...
                                if (GetColumnsInfo()[j].m_included)
                                    {
                                    const auto yCoord = bodyStart + GetCoulmnHeight() + GetCellTopPadding() +
                                    ...
                                    const auto cellTextDrawingRect =
                                        wxRect(currentX, yCoord,
                                               ...
                                               GetLineHeight() -
                                                   (2*GetCellTopPadding()) - m_list->GetDPIScaleFactor()/*border*/);
                                    ...
                                    if (GetColumnsInfo()[j].m_multiline)
                                        ...
                                        // then it needs to be split up and drawn as multiline text
                ...
                const auto drawHeadersAndFooters =
                    ...
                    if (m_list->GetLeftPrinterHeader().length() ||
                        ...
                        if (m_list->GetCenterPrinterHeader().length())
                            ...
                            drawDC.GetTextExtent(
                                ExpandPrintString(m_list->GetCenterPrinterHeader()), &textWidth, &textHeight);
                        ...
                        if (m_list->GetRightPrinterHeader().length())
                            {
                            drawDC.GetTextExtent(
                                ExpandPrintString(m_list->GetRightPrinterHeader()), &textWidth, &textHeight);
                    ...
                    if (m_list->GetLeftPrinterFooter().length() ||
                        ...
                        if (m_list->GetCenterPrinterFooter().length())
                            {
                            drawDC.GetTextExtent(
                                ExpandPrintString(m_list->GetCenterPrinterFooter()), &textWidth, &textHeight);
                        ...
                        if (m_list->GetRightPrinterFooter().length())
                            {
                            drawDC.GetTextExtent(
                                ExpandPrintString(m_list->GetRightPrinterFooter()), &textWidth, &textHeight);
                ...
                Wisteria::Canvas::DrawWatermarkLabel(gcdc,
                    wxRect(wxSize(drawingWidth, drawingHeight)), m_list->GetWatermark());
    ...
    void OnPreparePrinting() final
        ...
        if (dc)
            ...
            wxCoord cellTextWidth{ 0 }, cellTextHeigth{ 0 },
                    columnHeaderWidth{ 0 }, columnHeaderHeight{ 0 };
            ...
            for (long columnCounter = 0; columnCounter < m_list->GetColumnCount(); ++columnCounter)
                ...
                if (m_list->GetImageList(wxIMAGE_LIST_SMALL) &&
                    m_list->GetImageList(wxIMAGE_LIST_SMALL)->GetImageCount())
                    {
                    longestCellText += m_list->GetImageList(wxIMAGE_LIST_SMALL)->GetSize().GetWidth() +
                                       GetCellSidePadding();
                    }
            ...
            const long rowsPerPage =
                std::max<long>(safe_divide<long>(drawingHeight-GetCoulmnHeight(), m_lineHeight), 1);
            const long tablesPerPage = std::max<long>(
                static_cast<long>(std::floor(safe_divide<double>(drawingWidth,
                                  GetTableWidth() + GetTablePadding()))),
                1);
            ...
            const auto linesNeededForTableCaption = std::max<long>(
                static_cast<long>(std::ceil(safe_divide<double>(
                    (dc->GetMultiLineTextExtent(GetTitle()).GetHeight()*2) + (GetCellTopPadding()*2),
                     GetLineHeight()))),
                1);
...
void ListCtrlEx::OnCopyFirstColumn([[maybe_unused]] wxCommandEvent& event)
    ...
    FormatToHtml(selectedFormattedText, false, ExportRowSelection::ExportSelected,
                 0, -1, 0, 0, false, true);
...
void ListCtrlEx::Copy(const bool onlyIncludeSelectedRows, const bool includeColumnHeaders)
    ...
    wxString selectedFormattedText;
    FormatToHtml(selectedFormattedText, false, onlyIncludeSelectedRows ?
...
void ListCtrlEx::OnColClick(wxListEvent& event)
    ...
    if (event.GetColumn() == GetSortedColumn())
        {
        m_sortedCols[0].second = (m_sortedCols[0].second == Wisteria::SortDirection::SortAscending) ?
            Wisteria::SortDirection::SortDescending : Wisteria::SortDirection::SortAscending;
    ...
    if (GetSortedColumn() >= 0)
        ...
        for (size_t i = 0; i < GetSortedColumns().size(); ++i)
            ...
            hdItem.fmt |= (GetSortedColumns().at(i).second ==
                           Wisteria::SortDirection::SortAscending) ? HDF_SORTDOWN : HDF_SORTUP;
...
void ListCtrlEx::FormatToHtml(wxString& outputText,
    ...
    for (const auto& colInfo : printOut.GetColumnsInfo())
        ...
        if (colInfo.m_included)
            ...
            colGroup += wxString::Format(L"\n        <col style='width:%dpx'>",
                static_cast<int>(std::ceil(safe_divide<double>(colInfo.m_width + (2*printOut.GetCellSidePadding()),
                                 GetDPIScaleFactor()))));
            }
        }
    colGroup += L"\n    </colgroup>";
    ...
    const auto formatRow =
        [this, &outputText, &encode, &itemText, firstColumn, lastColumn]
        ...
        wxString rowStyle = (rowHeight == -1) ? wxString{} : wxString::Format(L"height:%dpx;", rowHeight);
        ...
        if (!GetAlternateRowColour().IsOk() &&
            ...
            {
            rowStyle += wxString::Format(_DT(L"background:%s;"),
    ...
    if (usePrinterSettings)
        ...
        for (size_t pageCounter = 0; pageCounter < printOut.GetPagesInfo().size(); ++pageCounter)
            ...
            for (size_t pageTableCounter = 0;
                 pageTableCounter < pageTable.m_rowStarts.size();
                 ++pageTableCounter)
                {
...
void ListCtrlEx::EditItem(const long selectedRow, const long selectedColumn)
    ...
    if (GetColumnEditMode(selectedColumn).m_editMode == ColumnInfo::ColumnEditMode::NoEdit)
        ...
        m_editComboBox = new ListEditComboBox(
            this, this, GetColumnEditMode(selectedColumn).m_selectableValues, wxID_ANY, wxString{},
            wxPoint(itemRect.x, itemRect.y), wxDefaultSize,
            GetColumnEditMode(selectedColumn).m_editMode == ColumnInfo::ColumnEditMode::ComboBoxEditReadOnly ?
                wxCB_DROPDOWN|wxCB_READONLY : wxCB_DROPDOWN);
        Select(selectedRow, false);
