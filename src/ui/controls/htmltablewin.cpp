...
namespace Wisteria::UI
    ...
    void HtmlTableWindow::OnPrint([[maybe_unused]] wxCommandEvent& event)
        ...
        while (tableStart != nullptr)
            {
            tableEnd = lily_of_the_valley::html_extract_text::find_closing_element(
                tableStart, htmlEnd, _DT(L"table"));
            if (tableEnd == nullptr)
                {
                break;
                }
    ...
    void HtmlTableWindow::OnSave([[maybe_unused]] wxCommandEvent& event)
        ...
        [[maybe_unused]]
        const bool result = Save(filePath);
        }
    ...
    void HtmlTableWindow::OnFind([[maybe_unused]] wxFindDialogEvent& event)
        {
        wxMessageBox(_(L"Find not supported in this window."), _(L"Find"),
