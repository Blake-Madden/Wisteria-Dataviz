...
long FormattedTextCtrl::FindText(const wchar_t* textToFind, const bool searchDown,
        ...
        [&selectionStart, &selectionEnd, &found, &selectionSearchPos, &matchWholeWord]()
    {
        ...
        else if (gtk_text_iter_starts_word(&selectionStart) &&
                 gtk_text_iter_ends_word(&selectionEnd))
            {
            return true;
            }
        // Searching for whole word, but this isn't it.
        // Step to the end of this match and search further.
        else
            {
            found = FALSE;
    ...
    if (searchDown)
        ...
        while (!found)
            {
            found = gtk_text_iter_forward_search(&selectionSearchPos, wxConvUTF8.cWC2MB(textToFind),
            ...
            if (!found &&
                ...
                found =
                    gtk_text_iter_forward_search(&textStart, wxConvUTF8.cWC2MB(textToFind), flags,
                                                 &selectionStart, &selectionEnd, &selStart);
                if (found)
                    {
                    if (confirmWholeWordMatch())
                        {
        ...
        while (!found)
            {
            found =
                gtk_text_iter_backward_search(&selectionSearchPos, wxConvUTF8.cWC2MB(textToFind),
                                              flags, &selectionStart, &selectionEnd, NULL);
            if (found)
                {
                if (confirmWholeWordMatch())
    ...
    if (found)
        ...
        SetSelection(gtk_text_iter_get_offset(&selectionStart),
