...
wxIMPLEMENT_DYNAMIC_CLASS(Wisteria::UI::FormattedTextCtrl, wxTextCtrl)
...
/** @brief Printing system for Windows.
                    ...
                    if (m_control->GetLeftPrinterFooter().length() ||
                        ...
                        if (m_control->GetRightPrinterFooter().length())
                            ...
                            dc.GetTextExtent(ExpandPrintString(m_control->GetRightPrinterFooter()),
                                             &textWidth, &textHeight);
                            dc.DrawText(
                                ExpandPrintString(m_control->GetRightPrinterFooter()),
    ...
    //------------------------------------------------------
    void FormattedTextCtrl::OnPrint([[maybe_unused]]
                                    wxCommandEvent &
                                    event)
        {
/* On Windows and macOS, we copy RTF designed for a white
   ...
   hidden RichEdit control.*/
        ...
        if (paperSize != nullptr)
            {
            gtk_page_setup_set_paper_size(pgSetup, paperSize);
            gtk_paper_size_free(paperSize);
            }
        ...
        if (printResult == GTK_PRINT_OPERATION_RESULT_APPLY)
            ...
            {
            wxMessageBox(
                wxString::Format(_(L"An error occurred while printing.\n%s"), error->message),
                _(L"Print"), wxOK | wxICON_QUESTION);

            g_error_free(error);
            }
        // NOLINTEND(cppcoreguidelines-pro-type-cstyle-cast,cppcoreguidelines-pro-type-vararg,bugprone-casting-through-void,modernize-type-traits)
    ...
    //------------------------------------------------------
    void FormattedTextCtrl::OnPreview([[maybe_unused]]
                                      wxCommandEvent &
                                      event)
        {
...
// built into its print dialog
    ...
    //------------------------------------------------------
    void FormattedTextCtrl::OnSave([[maybe_unused]]
                                   wxCommandEvent &
                                   event)
        {
        ...
        descriptions.Add(
            ...
            _(L"This format can be displayed in Internet browsers and most "
    ...
    //-----------------------------------------------------------
    void FormattedTextCtrl::OnContextMenu([[maybe_unused]]
                                          wxContextMenuEvent &
                                          event)
        {
    ...
    //------------------------------------------------------
    void FormattedTextCtrl::OnSelectAll([[maybe_unused]]
                                        wxCommandEvent &
                                        event)
        {
        SelectAll();
    ...
    //------------------------------------------------------
    void FormattedTextCtrl::OnCopyAll([[maybe_unused]]
                                      wxCommandEvent &
                                      event)
        {
        wxString formattedText;
#if defined(__WXMSW__) || defined(__WXOSX__)
        formattedText = !GetUnthemedFormattedText().empty() ? GetUnthemedFormattedTextRtf() :
                                                              GetFormattedTextRtf();
    ...
    wxString FormattedTextCtrl::GetUnthemedFormattedTextHtml(
        ...
        // Should be 7-bit ASCII, but if not then at least convert to something usable and parse
        // that. Using mb_str() uses the system locale, which isn't deterministic and can completely
        // fail if this isn't valid 7-bit ASCII.
        ...
        wxString text = wxString::Format(
            ...
            filterRtf.get_font_size(), filterRtf.get_font_color().red,
            filterRtf.get_font_color().green, filterRtf.get_font_color().blue);
    ...
    //-----------------------------------------------------------
    wxString FormattedTextCtrl::GetFormattedTextRtf(
        [[maybe_unused]] const bool fixHighlightingTags /*= true*/)
        {
#ifdef __WXGTK__
    ...
    wxString FormattedTextCtrl::GtkGetFormattedText(const GtkFormat format,
        ...
        gchar* bufferedUTF8Text = gtk_text_buffer_get_text(buffer, &start, &end, 0);
        if (bufferedUTF8Text == nullptr)
            {
            return {};
            }
        ...
        // NOLINTBEGIN(cppcoreguidelines-pro-type-cstyle-cast,cppcoreguidelines-pro-type-vararg,hicpp-vararg)
        // handle the first tag that defines the default formatting for all the text
        if (gtk_text_iter_starts_tag(&start, nullptr) != 0)
            {
        ...
        while (gtk_text_iter_compare(&start, &end) < 0)
            {
            if (gtk_text_iter_forward_to_tag_toggle(&start, nullptr) == 0)
                {
                break;
                }
            // get all the tags at the current iterator (there might be more than one)
            GSList *tags = nullptr, *tagp = nullptr;
            tags = gtk_text_iter_get_toggled_tags(&start, 0);
            tags = g_slist_concat(tags, gtk_text_iter_get_toggled_tags(&start, 1));
            ...
            for (tagp = tags; tagp != nullptr; tagp = tagp->next)
                ...
                if (gtk_text_iter_starts_tag(&start, tag) != 0)
                    {
                    if (format == GtkFormat::HtmlFormat)
                        {
                ...
                // (there might be more than one, though unlikely)
                else if (gtk_text_iter_ends_tag(&start, tag) != 0)
                    {
                    if (format == GtkFormat::HtmlFormat)
                        {
                        ...
                        currentTagText +=
                            wxString::Format(
                                L" \\strike0\\highlight0\\cf0\\ulnone\\b0\\i0\\f0\\fs%u ",
        ...
        // NOLINTEND(cppcoreguidelines-pro-type-cstyle-cast,cppcoreguidelines-pro-type-vararg,hicpp-vararg)
        ...
        if (format == GtkFormat::HtmlFormat)
            ...
            if (!colorTable.empty())
                ...
                for (const auto& colorPos : colorTable)
                    {
