...
void ImageMergeDlg::CreateControls(const wxArrayString& imgPaths)
    ...
    for (const auto& imgPath : imgPaths)
        {
        m_horizontalThumbsSizer->Add(new Wisteria::UI::Thumbnail(
            m_horizontalThumbsSizer->GetStaticBox(), Wisteria::GraphItems::Image::LoadFile(imgPath),
    ...
    for (const auto& imgPath : imgPaths)
        {
        m_verticalThumbsSizer->Add(new Wisteria::UI::Thumbnail(
            m_verticalThumbsSizer->GetStaticBox(), Wisteria::GraphItems::Image::LoadFile(imgPath),
            Wisteria::ClickMode::BrowseForImageFile, true));
        }
    ...
    Bind(
        ...
        [this]([[maybe_unused]] wxCommandEvent)
        {
            ...
            if (m_orientRadio == 0)
                {
                Wisteria::GraphItems::Image::StitchHorizontally(images).SaveFile(m_mergedFilePath);
                }
            else
                {
                Wisteria::GraphItems::Image::StitchVertically(images).SaveFile(m_mergedFilePath);
                }
