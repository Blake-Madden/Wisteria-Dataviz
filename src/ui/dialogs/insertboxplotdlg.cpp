...
namespace Wisteria::UI
    {
    //-------------------------------------------
    InsertBoxPlotDlg::InsertBoxPlotDlg(Canvas* canvas, const ReportBuilder* reportBuilder,
    ...
    void InsertBoxPlotDlg::CreateControls()
        ...
        GetSideBarBook()->AddPage(optionsPage, _(L"Box Plot Options"), ID_OPTIONS_SECTION, true);
        ...
        optionsSizer->Add(m_midpointCheck, wxSizerFlags{}.Border(wxLEFT));
    ...
    //-------------------------------------------
    BoxEffect InsertBoxPlotDlg::BoxEffectFromIndex(const int index) noexcept
        {
        switch (index)
            {
        case 1:
    ...
    //-------------------------------------------
    int InsertBoxPlotDlg::BoxEffectToIndex(const BoxEffect effect) noexcept
        {
        switch (effect)
            {
    ...
    //-------------------------------------------
    void InsertBoxPlotDlg::OnBoxEffectChanged()
        {
    ...
    //-------------------------------------------
    void InsertBoxPlotDlg::OnSelectStippleShape()
        {
    ...
    //-------------------------------------------
    void InsertBoxPlotDlg::OnSelectImages()
        {
    ...
    //-------------------------------------------
    void InsertBoxPlotDlg::OnDatasetChanged()
        {
    ...
    //-------------------------------------------
    void InsertBoxPlotDlg::OnSelectVariables()
        {
    ...
    //-------------------------------------------
    void InsertBoxPlotDlg::UpdateVariableLabels()
        {
        m_continuousVarLabel->SetLabel(m_continuousVariable);
        m_groupVarLabel->SetLabel(m_groupVariable);

        if (m_midpointCheck != nullptr)
            {
            m_midpointCheck->Enable(!m_groupVariable.empty());
            }
    ...
    InsertBoxPlotDlg::BuildColumnPreviewInfo(const Data::Dataset& dataset) const
        {
    ...
    //-------------------------------------------
    std::shared_ptr<Data::Dataset> InsertBoxPlotDlg::GetSelectedDataset() const
        {
    ...
    //-------------------------------------------
    bool InsertBoxPlotDlg::Validate()
        {
    ...
    void InsertBoxPlotDlg::LoadFromGraph(const Graphs::Graph2D& graph, Canvas* canvas)
        ...
        const auto dsName = boxPlot->GetPropertyTemplate(L"dataset");
        ...
        m_continuousVariable = boxPlot->GetPropertyTemplate(L"variables.aggregate");
        m_groupVariable = boxPlot->GetPropertyTemplate(L"variables.group-1");
        UpdateVariableLabels();

        // box options
        m_boxEffectIndex = BoxEffectToIndex(boxPlot->GetBoxEffect());
        m_showAllPoints = boxPlot->IsShowingAllPoints();
        m_showLabels = boxPlot->IsShowingLabels();

        // stipple shape (restore from property templates so it persists
        // even when the current box effect is not StippleShape)
        const auto savedShape = boxPlot->GetPropertyTemplate(L"stipple-shape");
        if (!savedShape.empty())
            {
        ...
        const auto savedColor = boxPlot->GetPropertyTemplate(L"stipple-shape-color");
        if (!savedColor.empty())
            {
            wxColour clr{ savedColor };
            if (clr.IsOk())
                {
        ...
        const auto savedPaths = boxPlot->GetPropertyTemplate(L"image-paths");
        if (!savedPaths.empty())
            {
        ...
        // image custom size
        const auto savedImgWidth = boxPlot->GetPropertyTemplate(L"image-width");
        const auto savedImgHeight = boxPlot->GetPropertyTemplate(L"image-height");
        ...
        // image resize method
        const auto savedResize = boxPlot->GetPropertyTemplate(L"image-resize-method");
        ...
        // image effect
        const auto savedEffect = boxPlot->GetPropertyTemplate(L"image-effect");
        ...
        const auto savedStitch = boxPlot->GetPropertyTemplate(L"image-stitch");
