...
namespace Wisteria::UI
    ...
    class InsertSankeyDiagramDlg final : public InsertGraphDlg
      ...
      public:
        ...
        /** @brief Constructs a Sankey diagram from the dialog's current settings.
            @param oldGraph The previous graph being edited, or @c nullptr if inserting new.
            @returns The newly constructed Sankey diagram.*/
        [[nodiscard]]
        std::shared_ptr<Graphs::SankeyDiagram>
        BuildSankeyDiagram(const Graphs::Graph2D* oldGraph = nullptr);
