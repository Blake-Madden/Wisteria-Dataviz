...
void FunctionBrowserDlg::FinalizeCategories()
    ...
    for (const auto& func : m_functionCollection)
        ...
        if (func.m_parentId == wxID_ANY)
            {
            m_categoryList->InsertItem(m_categoryList->GetFolderCount(),
                                       func.m_name, wxID_ANY, func.m_iconIndex);
            }
        else
            {
            m_categoryList->InsertSubItemById(func.m_parentId, func.m_name,
                                              wxID_ANY, func.m_iconIndex);
            }
...
void FunctionBrowserDlg::OnListSelected(wxCommandEvent& event)
    ...
    if (event.GetId() == ID_CATEGORY_LIST)
        ...
        if (m_functionList->GetCount())
            {
            m_functionList->SetSelection((pos->m_lastSelectedItem == -1) ? 0 : pos->m_lastSelectedItem);
            m_functionDescriptionWindow->SetPage(
                wxString::Format(L"<body bgcolor=%s text=%s>%s<br />%s</body>",
        ...
        if (m_functionList->GetSelection() < 0 ||
            static_cast<size_t>(m_functionList->GetSelection()) >= m_currentFunctionsAndDescriptions.size())
            { m_functionDescriptionWindow->SetPage(wxEmptyString); }
        else
            {
...
void FunctionBrowserDlg::InsertFunction()
    ...
    if (m_editWindow->IsKindOf(CLASSINFO(wxStyledTextCtrl)))
        ...
        wxString functionStr = m_currentFunctionsAndDescriptions[m_functionList->GetSelection()].first;
...
bool FunctionBrowserDlg::Create(wxWindow* parent, wxWindowID id /*= wxID_ANY*/,
    ...
    Move(wxSystemSettings::GetMetric(wxSYS_SCREEN_X) - (GetSize().GetWidth() +
         wxSizerFlags::GetDefaultBorder()), GetScreenPosition().y);
...
void FunctionBrowserDlg::CreateControls(const wxString& firstWindowCaption,
    ...
    m_functionList = new wxListBox(this, ID_FUNCTION_LIST,
        wxDefaultPosition, FromDIP(wxSize(275, 400)), 0, nullptr,
        wxBORDER_THEME|wxLB_SINGLE|wxLB_HSCROLL|wxLB_NEEDED_SB);
    ...
    m_functionDescriptionWindow = new wxHtmlWindow(this, wxID_ANY, wxDefaultPosition,
