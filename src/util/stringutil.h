...
        @param value The integer to convert.
    ...
    /** @brief Searches for a matching tag, skipping an extra open/close pairs of symbols in between.
    ...
    /** @brief Searches in a string for a single character from a larger sequence and returns a pointer if found.
        @param stringToSearch The string to search in.
    ...
    /** @brief Searches for a single character from a sequence in a string for n number of characters.
        @param stringToSearch The string to search.
    ...
    [[nodiscard]] size_t find_first_not_of(const T* stringToSearch, const size_t stringToSearchLength,
        ...
        for (i = 0; i < stringToSearchLength; ++i)
            ...
            /* If string being searched is shorter than the size argument then return string
               length to indicate failure to find anything.
               Although this isn't really the index that we stopped in, this will tell the
    ...
    public:
        ...
        string_tokenize(const T& val, const std::wstring& delims, const bool skipEmptyTokens) noexcept :
            ...
            m_next_delim = string_util::strcspn_pointer(m_start, m_delims.c_str(), m_delims.length());
            ...
            for (size_t i = 0; i < val.length(); ++i)
                ...
                if (is_one_of<wchar_t>(val[i], m_delims.c_str()) )
                    ...
                    if (m_skip_empty_tokens)
                        {
                        if (i+1 < val.length() && is_one_of<wchar_t>(val[i+1], m_delims.c_str()) )
                            { continue; }
                        else
                            { ++tokenCount; }
                        }
