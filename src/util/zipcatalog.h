...
namespace Wisteria
    ...
    public:
        ...
        [[nodiscard]] wxZipEntry* Find(const wxString& value) const
            {
            std::map<wxString, wxZipEntry*>::const_iterator pos = m_catalog.find(wxZipEntry::GetInternalName(value));
            return (pos == m_catalog.end() ) ? nullptr : pos->second;
            }
        ...
        [[nodiscard]] wxArrayString FindFilesInFolder(const wxString& path) const;
        /** @returns A list of all files (relative to its location in the ZIP folder structure).*/
        [[nodiscard]] wxArrayString GetPaths() const;
        /** Reads a text file in the archive and returns its content. File will be converted
                to unicode, so this should only be called on actual text files
                (e.g., *.txt, *.htm, etc.).
            @param path The path to the file (relative to its location in the ZIP folder structure).
            @returns The text file's content.*/
        [[nodiscard]] wxString ReadTextFile(const wxString& path) const;
        ...
        /** @brief Reads an image from the ZIP file.
            ...
            @returns The bitmap from the ZIP file, or wxNullBitmap if not found.*/
        [[nodiscard]] wxBitmap ReadBitmap(const wxString& path, const wxBitmapType bitmapType) const;
        ...
        [[nodiscard]] wxString ExtractTextFileToTempFile(const wxString& path) const;
        ...
        static void WriteText(wxZipOutputStream& zip, const wxString& fileName, const wxString& text);
        /// @returns True if any read operations had failed, usually due to corruption or password protection.
        [[nodiscard]] bool HadReadErrors() const noexcept
            { return m_readErrorShown; }
        /// @returns The messages encountered while loading files from the archive.
        [[nodiscard]] const std::vector<ArchiveMessage>& GetMessages() const noexcept
            { return m_messages; }
        /// @brief Clears all of the logged messages from previous reads.
        void ClearMessages() noexcept
            { m_messages.clear(); }
    private:
        /// @brief Optimized reading function which can use a large buffer for larger compressed files.
        bool Read(wxInputStream* stream_in, wxOutputStream& stream_out, const size_t bufferSize) const;
