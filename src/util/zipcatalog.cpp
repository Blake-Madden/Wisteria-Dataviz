...
namespace Wisteria
    ...
    bool ZipCatalog::Read(wxInputStream* stream_in, wxOutputStream& stream_out,
        ...
        if (totalWritten == 0 && !m_readErrorShown)
            {
            m_messages.emplace_back(_(L"Unable to read file from archive. Archive may be corrupt "
                                      L"or password protected."),
                                    wxICON_EXCLAMATION);
    ...
    //------------------------------------------------
    void ZipCatalog::WriteText(wxZipOutputStream& zip, const wxString& fileName,
                               const wxString& text)
        {
