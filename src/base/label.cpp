...
namespace Wisteria::GraphItems
    ...
    void Label::SetBoundingBox(const wxRect& rect, wxDC& dc,
        ...
        //---------------
        // In horizontal orientation, the top image adds to height; in vertical orientation
        // it adds to width. Subtract that space before passing to CalcLeftImageSize so the
        // left image scales against the text area only, not the whole label.
        const auto topImgSpace =
            std::max<wxCoord>(CalcTopGraphicSize().GetHeight() - m_topImageOffset, 0);
        // left image
        auto measureWidthNoSideImages =
            ((GetTextOrientation() == Orientation::Horizontal) ?
                 (measuredWidth - CalcLeftImageSize(measureHeight - topImgSpace).GetWidth()) :
                 measuredWidth);
        auto measuredHeightNoSideImages =
            ((GetTextOrientation() == Orientation::Vertical) ?
                 (measureHeight - CalcLeftImageSize(measuredWidth - topImgSpace).GetWidth()) :
                 measureHeight);
        // top image
        measuredHeightNoSideImages -=
            ((GetTextOrientation() == Orientation::Horizontal) ? topImgSpace : 0);
        measureWidthNoSideImages -=
            ((GetTextOrientation() == Orientation::Vertical) ? topImgSpace : 0);

        auto textAreaWidthNoSideImages =
            ((GetTextOrientation() == Orientation::Horizontal) ?
                 (rect.GetWidth() - CalcLeftImageSize(rect.GetHeight() - topImgSpace).GetWidth()) :
                 rect.GetWidth());
        auto textAreaHeightNoSideImages =
            ((GetTextOrientation() == Orientation::Vertical) ?
                 (rect.GetHeight() - CalcLeftImageSize(rect.GetWidth() - topImgSpace).GetWidth()) :
                 rect.GetHeight());
        // top image
    ...
    wxRect Label::GetBoundingBox(wxDC& dc) const
        ...
        const wxCoord width = std::max<wxCoord>(
            measuredWidth, ScaleToScreenAndCanvas(GetMinimumUserWidthDIPs().value_or(0)));
        ...
        if (GetTextOrientation() == Orientation::Horizontal)
            ...
            if (GetAnchoring() == Anchoring::Center)
                ...
                {
                boundingBox =
                    wxRect(GetAnchorPoint() - wxPoint(width, height), wxSize(width, height));
                }
            }
        else
            {
    ...
    void Label::GetSize(wxDC& dc, wxCoord& width, wxCoord& height) const
        ...
        if (GetTextOrientation() == Orientation::Horizontal)
            ...
            height += ScaleToScreenAndCanvas(GetLeftPadding()) +
                      ScaleToScreenAndCanvas(GetRightPadding());
            width += spaceBetweenLines + ScaleToScreenAndCanvas(GetTopPadding()) +
                     ScaleToScreenAndCanvas(GetBottomPadding());
            if (GetHeaderInfo().IsEnabled() && firstLineEnd != std::wstring::npos &&
                secondLineStart != std::wstring::npos)
                {
            ...
            for (auto iconPos = GetLegendIcons().cbegin(); iconPos != GetLegendIcons().cend();
                ...
                const wxCoord yOffset =
                    (GetHeaderInfo().IsEnabled() ?
                         topLineHeight + std::ceil(ScaleToScreenAndCanvas(GetLineSpacing())) :
                         0);
                    ...
                    case Icons::IconShape::ColorGradient:
                        if (iconPos->m_colors.size() >= 2)
                            ...
                            const wxCoord chunkHeight =
                                safe_divide(legendArea.GetHeight(), chunkSections);
                            for (size_t i = 0; i < iconPos->m_colors.size() - 1; ++i)
                                ...
                                dc.GradientFillLinear(currentLegendChunk, iconPos->m_colors[i],
                                                      iconPos->m_colors[i + 1], wxDOWN);
                                }
                            }
                        break;
                    default:
    ...
    wxRect Label::Draw(wxDC& dc) const
            ...
            if (GetBoxCorners() == BoxCorners::Rounded)
                ...
                dc.DrawRoundedRectangle(
                    wxRect(boundingBox.GetLeftTop() +
                           ...
                           boundingBox.GetSize()),
                    Settings::GetBoxRoundedCornerRadius());
                }
            else
                {
                dc.DrawRectangle(wxRect(boundingBox.GetLeftTop() +
                                            wxPoint(ScaleToScreenAndCanvas(GetShadowOffset()),
                                                    ScaleToScreenAndCanvas(GetShadowOffset())),
                                        boundingBox.GetSize()));
                }
        ...
        if (m_leftImage.IsOk())
            ...
            if (GetTextOrientation() == Orientation::Horizontal)
                {
                // subtract top-image space so the left image is sized against the text
                // area height, not the whole label (which may include a tall top image)
                const auto topImgSpace =
                    std::max<wxCoord>(CalcTopGraphicSize().GetHeight() - m_topImageOffset, 0);
                const auto bmp = m_leftImage.GetBitmap(
                    CalcLeftImageSize(GetCachedContentBoundingBox().GetHeight() - topImgSpace));
                // center vertically in the text area (below the top image)
                auto leftCorner{ GetCachedContentBoundingBox().GetTopLeft() };
                leftCorner.y +=
                    topImgSpace +
                    safe_divide(GetCachedContentBoundingBox().GetHeight() - topImgSpace, 2) -
                ...
                const auto img =
                    m_leftImage
                        .GetBitmap(CalcLeftImageSize(GetCachedContentBoundingBox().GetHeight()))
                        .ConvertToImage()
                        .Rotate90(false);
                // center horizontally
    ...
    bool Label::SplitTextByListItems()
        ...
        // and newline after
        const wxRegEx reCommaSep(wxString::Format(L", ([^%s])", andOrString));
    ...
    bool Label::SplitTextAuto()
        ...
        if (const auto charPos = splitText.find_first_of(L"([/:&");
            charPos != std::wstring::npos && charPos != splitText.length() - 1 && charPos != 0)
            {
            if ((splitText[charPos] == L':' || splitText[charPos] == L'&') &&
                ...
                {
                splitText.insert(((splitText[charPos] == L':' || splitText[charPos] == L'&') ?
                                      charPos + 1 :
                                      charPos),
                                 1, L'\n');
                }
    ...
    void Label::SplitTextToFitLength(const size_t suggestedLineLength)
        ...
        while (tempStr.length() > suggestedLineLength)
            ...
            if (index != std::wstring::npos)
                {
                fittedText
                    .append(std::wstring_view(tempStr.wc_str()).substr(0, index + 1).data(),
                            index + 1)
                    .Trim(true)
                    .append(L'\n');
        ...
        fittedText.Trim(true);
        fittedText.Trim(false);
        SetText(fittedText);
        }
    ...
    void Label::DrawVerticalMultiLineText(wxDC& dc, wxPoint pt) const
        ...
        // render the text
        wxCoord lineX{ 0 }, lineY{ 0 }, offset{ 0 };
        ...
        if (GetTextAlignment() == TextAlignment::JustifiedAtCharacter ||
            ...
            fullTextSz.SetHeight(fullTextSz.GetHeight() -
                                 (spaceBetweenLines + ScaleToScreenAndCanvas(GetLeftPadding()) +
                                  ScaleToScreenAndCanvas(GetRightPadding())));
            }
        ...
        // vertical orientation: top-image space is in the width dimension, so subtract
        // before passing to CalcLeftImageSize
        const auto topImgSpaceForLeft =
            std::max<wxCoord>(CalcTopGraphicSize().GetHeight() - m_topImageOffset, 0);
        fullTextSz.SetHeight(
            fullTextSz.GetHeight() -
            CalcLeftImageSize(GetCachedContentBoundingBox().GetWidth() - topImgSpaceForLeft)
                .GetWidth());
        fullTextSzForHeader.SetHeight(
            fullTextSzForHeader.GetHeight() -
            CalcLeftImageSize(GetCachedContentBoundingBox().GetWidth() - topImgSpaceForLeft)
                .GetWidth());
            ...
            [&dc, &tokenizedLineLetters](wxString& textLine, const wxSize textSz, bool atCharacter)
        {
            ...
            if (dc.GetTextExtent(textLine).GetWidth() < textSz.GetHeight())
                ...
                if (atCharacter)
                    ...
                    wxStringTokenizer tkzr(textLine, L" ",
                                           wxStringTokenizerMode::wxTOKEN_RET_DELIMS);
                ...
                const auto hairSpacesPerWordPair =
                    std::max(1.0, std::floor(safe_divide<double>(hairSpacesNeeded, letterSpaces)));
        ...
        while (lineTokenizer.HasMoreTokens())
            ...
            if (GetHeaderInfo().IsEnabled() && currentLineNumber == 0 && GetLineCount() > 1)
                ...
                if (GetTextAlignment() == TextAlignment::FlushLeft)
                    {
                    offset = ScaleToScreenAndCanvas(GetLeftPadding());
                    }
                else if (GetTextAlignment() == TextAlignment::Centered)
                    {
                    offset = (safe_divide<double>(fullTextSz.GetHeight(), 2) -
                              safe_divide<double>(lineX, 2));
                    }
                else if (GetTextAlignment() == TextAlignment::FlushRight)
                    {
                    offset =
                        fullTextSz.GetHeight() - lineX - ScaleToScreenAndCanvas(GetLeftPadding());
                    }
            ...
            const auto xOffset =
                std::max<wxCoord>(CalcTopGraphicSize().GetHeight() - m_topImageOffset, 0);
            // vertical orientation: subtract top-image space (in width dim) before
            // passing to CalcLeftImageSize
            const auto currentLineOffset =
                (GetLinesIgnoringLeftMargin().contains(currentLineNumber)) ?
                    ...
                    ((GetHeaderInfo().IsEnabled() && currentLineNumber == 0) ? 0 : leftOffset) +
                        CalcLeftImageSize(GetCachedContentBoundingBox().GetWidth() - xOffset)
                            .GetWidth();
    ...
    void Label::DrawMultiLineText(wxDC& dc, wxPoint pt) const
        ...
        if (GetTextAlignment() == TextAlignment::JustifiedAtCharacter ||
            ...
            fullTextSz.SetWidth(fullTextSz.GetWidth() -
                                (ScaleToScreenAndCanvas(GetLeftPadding()) +
                                 ScaleToScreenAndCanvas(GetRightPadding())));
            fullTextSz.SetHeight(fullTextSz.GetHeight() -
                                 (spaceBetweenLines + ScaleToScreenAndCanvas(GetTopPadding()) +
                                  ScaleToScreenAndCanvas(GetBottomPadding())));
            }
        if (GetHeaderInfo().GetLabelAlignment() == TextAlignment::JustifiedAtCharacter ||
            GetHeaderInfo().GetLabelAlignment() == TextAlignment::JustifiedAtWord)
        ...
        // horizontal orientation: top-image space is in the height dimension, so subtract
        // before passing to CalcLeftImageSize
        const auto topImgSpaceForLeft =
            std::max<wxCoord>(CalcTopGraphicSize().GetHeight() - m_topImageOffset, 0);
        fullTextSz.SetWidth(
            fullTextSz.GetWidth() -
            CalcLeftImageSize(GetCachedContentBoundingBox().GetHeight() - topImgSpaceForLeft)
                .GetWidth());
        fullTextSzForHeader.SetWidth(
            fullTextSzForHeader.GetWidth() -
            CalcLeftImageSize(GetCachedContentBoundingBox().GetHeight() - topImgSpaceForLeft)
                .GetWidth());
        ...
        while (lineTokenizer.HasMoreTokens())
            ...
            if (GetHeaderInfo().IsEnabled() && currentLineNumber == 0 && GetLineCount() > 1)
                ...
                // note that for centering we need to add half of the margin back in
                    ...
                    {
                    offset = (safe_divide<double>(fullTextSzForHeader.GetWidth(), 2) -
                              safe_divide<double>(lineX, 2)) +
                             leftOffset;
                    }
                else if (GetHeaderInfo().GetLabelAlignment() == TextAlignment::FlushRight)
                    {
                    offset = (fullTextSzForHeader.GetWidth() - lineX -
                              ScaleToScreenAndCanvas(GetLeftPadding())) +
                             (HasLegendIcons() ? 0 : leftOffset);
                    }
                else if (GetHeaderInfo().GetLabelAlignment() == TextAlignment::JustifiedAtCharacter)
                ...
                if (GetTextAlignment() == TextAlignment::FlushLeft)
                    ...
                    {
                    offset = (safe_divide<double>(fullTextSz.GetWidth(), 2) -
                              safe_divide<double>(lineX, 2));
                    }
                else if (GetTextAlignment() == TextAlignment::FlushRight)
                    {
                    offset =
                        fullTextSz.GetWidth() - lineX - ScaleToScreenAndCanvas(GetRightPadding());
                    }
                else if (GetTextAlignment() == TextAlignment::JustifiedAtCharacter)
                    {
                    offset = ScaleToScreenAndCanvas(GetLeftPadding());
            ...
            const auto yOffset =
                std::max<wxCoord>(CalcTopGraphicSize().GetHeight() - m_topImageOffset, 0);
            // horizontal orientation: subtract top-image space (in height dim) before
            // passing to CalcLeftImageSize
            const auto currentLineOffset =
                (GetLinesIgnoringLeftMargin().contains(currentLineNumber)) ?
                    0 :
                    ((GetHeaderInfo().IsEnabled() && currentLineNumber == 0) ? 0 : leftOffset) +
                        CalcLeftImageSize(GetCachedContentBoundingBox().GetHeight() - yOffset)
                            .GetWidth();
            const bool isHeader{ (currentLineNumber == 0 && GetLineCount() > 1 &&
                                  GetHeaderInfo().IsEnabled() &&
                                  GetHeaderInfo().GetFont().IsOk()) };
    ...
    void Label::FixFont(wxFont& theFont)
        ...
        if (theFont.GetFaceName() == L".Lucida Grande UI")
            {
            theFont.SetFaceName(
                wxFontEnumerator::IsValidFacename(L"Lucida Grande") ?
                    wxString(L"Lucida Grande") :
        ...
        // OSX 10.10 font
            ...
            {
            theFont.SetFaceName(
                wxFontEnumerator::IsValidFacename(L"Helvetica Neue") ?
        ...
        if (!theFont.IsOk())
            ...
            wxLogWarning(L"Unable to correct font '%s'; will be replaced by system default.",
                         theFont.GetFaceName());
            }
        if (originalFaceName != theFont.GetFaceName())
            {
            wxLogMessage(L"Font remapped from '%s' to '%s'.", originalFaceName,
                         theFont.GetFaceName());
            }
        }
