...
namespace Wisteria
    ...
    public:
        ...
        void AddTable(const std::shared_ptr<Graphs::Table> table)
        ...
        void SyncTableSizes()
            ...
            const auto maxRowCount =
                std::max_element(m_tables.cbegin(),
                    m_tables.cend(),
                    [](const auto& lhv, const auto& rhv) noexcept
                    { return lhv->GetTableSize().first < rhv->GetTableSize().first; })->get()->
                GetTableSize().first;
            const auto maxColumnCount =
                std::max_element(m_tables.cbegin(),
                    m_tables.cend(),
                    [](const auto& lhv, const auto& rhv) noexcept
                    { return lhv->GetTableSize().second < rhv->GetTableSize().second; })->get()->
                GetTableSize().second;
