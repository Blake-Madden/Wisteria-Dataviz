      ...
      public:
        ...
        static int GetPolygonWidth(const polygonT& polygon)
            ...
            for (auto pt : polygon)
                {
                auto startX{ pt.x };
                while (geometry::is_inside_polygon(pt, polygon))
                    {
                    ++pt.x;
                    }
        ...
        [[nodiscard]] [[deprecated("Use version geometry::is_inside_polygon()")]]
        ...
        static bool IsRectInsidePolygon(const wxRect rect, const polygonT& polygon)
            {
            return (geometry::is_inside_polygon(rect.GetTopLeft(), polygon) &&
                    geometry::is_inside_polygon(rect.GetTopRight(), polygon) &&
                    geometry::is_inside_polygon(rect.GetBottomLeft(), polygon) &&
                    geometry::is_inside_polygon(rect.GetBottomRight(), polygon));
            }
