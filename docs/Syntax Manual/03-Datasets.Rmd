...
- `"datasets"`: contains an array of datasets, which are referenced by other items in the report.
  ...
  - `"md-codes"`: an array of string values to import as missing data during the import.<br />
    This property is optional.
  ...
  - "treat-years-as-text": whether to import numeric columns that are four-digits as text
    (going under the assumption that they are years).<br />
    This property is optional. Also, this is only used if you are auto-detecting the columns and their types.
  ...
  - `"columns-select"`: the columns to select (using a regular expression pattern).<br />
  ...
  - `"id-columns"`: an array of strings representing the ID columns.<br />
