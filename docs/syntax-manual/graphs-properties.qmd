...
- `"background-image"`: An object specifying an image to draw within the plotting area. \
    Contains the following properties:
    - `"image-import"`: An [image file specification](#image-file-properties) for the image
        (supports single path, multiple paths with stitching, and effects).
    - `"size"`: An optional object with `"width"` and `"height"` to override the default size.
    - `"opacity"`: An integer (0–255) controlling the image's opacity.
       Default is 255 (fully opaque).
    - `"image-fit"`: How the image fits inside the plot area.
       Options are:
        - `"shrink"` (default): Shrink the image to fit the plot area.
        - `"crop-and-center"`: Crop the image to the plot area dimensions,
            using the center of the image as the crop origin.
...
- `"annotations"`: Text messages that can be added on top of the graph, optionally pointing to positions on the graph. \
  ...
  - `"anchor"`: The point to anchor the label. \
     Note that the `label`'s `"anchoring"` property will control which part of the label that the anchoring point refers to. \
...
- `"reference-areas"`: An array of reference area definitions, which contain the following properties:
  ...
  - `"style"`: The style to apply to the reference area. \
    ...
    - `"fade-from-left-to-right"`: Fades from the start value to the end value.
    - `"fade-from-right-to-left"`: Fades from the end value to the start value. \
...
- `"legend"`: An item containing the following properties:
  - `"placement"`: Where to place the legend. \
    Available options are:
    - `"left"`: The legend will be on the left of the plot.
    - `"right"`: The legend will be on the right of the plot (the default).
    - `"top"`: The legend will be above the plot.
    - `"bottom"`: The legend will be below the plot.
...
- `"stitch"`: A string specifying how to stitch multiple images together.
  ...
  Available options are:
  - `"horizontal"`: Stitch images left to right (the default).
  - `"vertical"`: Stitch images top to bottom.
...
- `"effect"`: A string specifying an effect to apply to the image. \
  Available options are:
  - `"no-effect"`: Do not alter the image.
  - `"gray-scale"`: Shades of gray (i.e., black & white).
  - `"blur-horizontal"`: A horizontal blur across the image.
  - `"blur-vertical"`: A vertical blur across the image.
  - `"sepia"`: A sepia (i.e., faded photograph) effect.
  - `"frosted-glass"`: A frosted glass window effect. \
     In other words, the image as it may appear when viewed through frosted glass.
  - `"oil-painting"`: An oil painting effect.
...
- `"tickmarks"`: Tickmark settings, which contains the following properties:
  - `"display"`: A string, indicating how to display the tickmarks. \
    ...
    - `"no-display"`: Do not display tickmarks.
- `"label-display"`: What to display for the labels along the axis. \
  Available options are:
  - `"custom-labels-or-values"`: Display either custom label (if available) or numeric value.
  - `"only-custom-labels"`: Only show as a custom label; nothing will be displayed if custom label isn't available.
  - `"custom-labels-and-values"`: Display both numeric value and custom label.
  - `"no-display"`: Do not display any label.
  - `"values"`: Display the numeric value.
...
- `"custom-labels"`: An array of custom label specifications, which contains the following properties:
  ...
  - `"label"`: The text to display next to the brackets, which will contain [label](#label-properties) properties.
- `"range"`: A node describing the range to use for the axis, which contains the following properties:
  ...
  - `"display-interval"`: A number specifying how often the axis points should show a label.
- `"precision"`: A number specifying the precision to use for numbers displayed on the axis. \
...
- `"brackets"`: Adds brackets to the axis, which can be an array of bracket definitions or a single set of properties
  using a dataset. \
  ...
  - `"style"`: The style of the braces. \
    Available options are:
    - `"lines"`: Regular line.
    - `"arrow"`: Arrow pointing at the axis.
    - `"reverse-arrow"`: Arrow pointing at the labels on the bracket.
    - `"curly-braces"`: The connecting lines and main bracket line will be a curly brace.
    - `"no-connection-lines"`: Do not draw a line from the axis to the bracket; only the bracket itself will be drawn.

  If building brackets individually, specify an array of items, each with the following properties:
  - `"start"`: The starting axis position for the area. \
       This can either be a numeric position, a date (as a string), or an axis label.
  - `"end"`: The ending axis position for the area. \
       This can either be a numeric position, a date (as a string), or an axis label.
  - `"label"`: The text to display next to the brackets.
  - `"pen"`: The bracket line, which includes [pen](#pen-properties) properties.
  - `"style"`: The style of the braces. \
    Available options are:
    - `"lines"`: Regular line.
    - `"arrow"`: Arrow pointing at the axis.
    - `"reverse-arrow"`: Arrow pointing at the labels on the bracket.
    - `"curly-braces"`: The connecting lines and main bracket line will be a curly brace (the default).
    - `"no-connection-lines"`: Do not draw a line from the axis to the bracket; only the bracket itself will be drawn.
