...
- `"max-discrete-value"`: The max value that can be seen as discrete when deducing between
  ...
  If an integer is larger then this value in a column, then it will classify the column as continuous. \
  This property is optional.
  Also, this is only used if you are auto-detecting the columns and their types.
...
- `"mutate-categorical-columns"`: An array of categorical column mutation commands. \
  Each command will either create or update a categorical columns and fill it with values based on the provided categorical column.
...
- `"fill-value"`: A numeric value to assign to missing data.
  The default it so leave it as missing data.
