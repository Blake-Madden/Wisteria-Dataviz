...
- `"variables"`: An item containing the following properties:
  ...
  - `"group"`: The grouping column (this is optional).
- `"showcase-score"`: `true` to make most areas of the chart translucent,
  except for where the score falls. This draws attention to the relevant
  areas of the scales. Default is `false`.
- `"main-scale-values"`: An array of numbers defining the tick marks
  on the main scale axis (e.g., `[10, 20, 30, 40, 50, 60, 70, 80, 90]`).
- `"main-scale-precision"`: The number of decimal places for the main
  scale values. Default is `0`.
- `"main-scale-header"`: The header label above the main scale column.
- `"data-column-header"`: The header label above the data column.
- `"scales"`: An array of scale objects. Each scale contains:
