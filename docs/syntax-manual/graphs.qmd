...
- `"ghost-non-showcased-groups"`: A boolean specifying whether the "to" column's boxes and labels are also ghosted for groups not included in `"showcase-streams"`. \
  This will only have an effect if `"showcase-streams"` is in use. Default is `false`.
:::
