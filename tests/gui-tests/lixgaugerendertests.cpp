///////////////////////////////////////////////////////////////////////////////
// Name:        lixgaugerendertests.cpp
// Purpose:     Characterization tests for LixGauge layout output
// Author:      Blake Madden
// Copyright:   (c) 2026 Blake Madden
// License:     3-Clause BSD license
///////////////////////////////////////////////////////////////////////////////

// These tests lock in the observable layout output of LixGauge: left-Y and
// bottom-X axis ranges and intervals, axis slot counts, whether grouping is in
// use, the subgroup count, and the number of render objects produced. LixGauge
// derives from GroupGraph2D and draws a fixed readability scale with the
// documents' scores plotted onto it. The assertions are invariance based; a
...
namespace
    ...
    [[nodiscard]]
    std::shared_ptr<LixGauge> BuildChart(Canvas* canvas, const ChartSpec& spec)
        {
        ...
        auto chart = std::make_shared<LixGauge>(canvas);
    ...
    [[nodiscard]]
    LayoutFingerprint LayOutAndCapture(Canvas* canvas, const std::shared_ptr<LixGauge>& chart)
        {
    ...
    std::vector<std::pair<std::string, ChartSpec>> AllSpecs()
        ...
        specs.push_back({ "tight", ChartSpec{ .m_scores = { 44, 45, 46 } } });
        return specs;
        }

    // Exact layout output recorded from the [.dump] test, keyed by spec name.
    // Update only for a deliberate, reviewed change to LixGauge layout.
    [[nodiscard]]
    LayoutFingerprint ExpectedFingerprint(const std::string& specName)
        ...
        if (specName == "two-groups")
            {
            return LayoutFingerprint{ { 10, 70, 5, 0, true, 13 },
                                      { 0, 2, 1, 0, false, 3 },
                                      true, 2, 10 };
            }
...
TEST_CASE("LixGauge layout is deterministic and idempotent", "[lixgauge][render]")
    {
...
TEST_CASE("LixGauge layout matches the recorded baseline", "[lixgauge][render]")
    {
    for (const auto& [name, spec] : AllSpecs())
        {
        SECTION(name)
...
TEST_CASE("LixGauge lays out the score scale with a positive interval", "[lixgauge][render]")
    {
    for (const auto& [name, spec] : AllSpecs())
        {
        SECTION(name)
...
TEST_CASE("LixGauge grouping flag tracks the group column", "[lixgauge][render]")
    {
    for (const auto& [name, spec] : AllSpecs())
        {
        SECTION(name)
...
// executable.
TEST_CASE("LixGauge characterization dump", "[lixgauge][render][.dump]")
    {
    const wxString outPath{ wxFileName(wxStandardPaths::Get().GetExecutablePath()).GetPath() +
                            L"/lixgauge_characterization.txt" };
