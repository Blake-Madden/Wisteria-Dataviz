///////////////////////////////////////////////////////////////////////////////
// Name:        lixgaugegermanrendertests.cpp
// Purpose:     Characterization tests for LixGaugeGerman layout output
// Author:      Blake Madden
// Copyright:   (c) 2026 Blake Madden
// License:     3-Clause BSD license
///////////////////////////////////////////////////////////////////////////////

// These tests lock in the observable layout output of LixGaugeGerman: left-Y
...
// LixGaugeGerman derives from GroupGraph2D and draws a fixed readability scale
// adapted for German materials with the documents' scores plotted onto it. The
...
#include "../../src/graphs/lixgaugegerman.h"
...
namespace
    ...
    struct ChartSpec
        {
        std::vector<double> m_scores{ 35, 42, 48 };
        std::vector<GroupIdType> m_groupCodes;
        ColumnWithStringTable::StringTableType m_groupLabels;
        bool m_useEnglishLabels{ false };
        };

    [[nodiscard]]
    std::shared_ptr<LixGaugeGerman> BuildChart(Canvas* canvas, const ChartSpec& spec)
        {
        ...
        auto chart = std::make_shared<LixGaugeGerman>(canvas);
        chart->UseEnglishLabels(spec.m_useEnglishLabels);
    ...
    [[nodiscard]]
    LayoutFingerprint LayOutAndCapture(Canvas* canvas, const std::shared_ptr<LixGaugeGerman>& chart)
        {
    ...
    std::vector<std::pair<std::string, ChartSpec>> AllSpecs()
        ...
        specs.push_back(
            { "spread", ChartSpec{ .m_scores = { 24, 33, 41, 50, 58, 45, 37 } } });
        specs.push_back({ "two-groups",
                          ChartSpec{ .m_scores = { 35, 55, 40, 58, 33, 52 },
                                     .m_groupCodes = { 0, 1, 0, 1, 0, 1 },
                                     .m_groupLabels = { { 0, L"A" }, { 1, L"B" } } } });
        specs.push_back({ "english-labels",
                          ChartSpec{ .m_scores = { 30, 44, 52 }, .m_useEnglishLabels = true } });
        return specs;
        }

    // Exact layout output recorded from the [.dump] test, keyed by spec name.
    // Update only for a deliberate, reviewed change to LixGaugeGerman layout.
    [[nodiscard]]
    LayoutFingerprint ExpectedFingerprint(const std::string& specName)
        ...
        if (specName == "two-groups")
            {
            return LayoutFingerprint{ { 20, 70, 5, 0, true, 11 },
                                      { 0, 2, 1, 0, false, 3 },
                                      true, 2, 8 };
            }
        if (specName == "single-band" || specName == "spread" ||
            specName == "english-labels")
            {
            return LayoutFingerprint{ { 20, 70, 5, 0, true, 11 },
                                      { 0, 2, 1, 0, false, 3 },
                                      false, 0, 8 };
...
TEST_CASE("LixGaugeGerman layout is deterministic and idempotent", "[lixgaugegerman][render]")
    {
...
TEST_CASE("LixGaugeGerman layout matches the recorded baseline", "[lixgaugegerman][render]")
    {
    for (const auto& [name, spec] : AllSpecs())
        {
        SECTION(name)
...
TEST_CASE("LixGaugeGerman lays out the score scale with a positive interval",
          "[lixgaugegerman][render]")
    {
    for (const auto& [name, spec] : AllSpecs())
        {
        SECTION(name)
            ...
            // The Lix score scale runs top-down, so the left-Y axis is reversed.
            CHECK(print.m_leftYAxis.m_reversed);
...
TEST_CASE("LixGaugeGerman grouping flag tracks the group column", "[lixgaugegerman][render]")
    {
    for (const auto& [name, spec] : AllSpecs())
        {
        SECTION(name)
...
// executable.
TEST_CASE("LixGaugeGerman characterization dump", "[lixgaugegerman][render][.dump]")
    {
    const wxString outPath{ wxFileName(wxStandardPaths::Get().GetExecutablePath()).GetPath() +
                            L"/lixgaugegerman_characterization.txt" };
