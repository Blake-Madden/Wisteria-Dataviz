///////////////////////////////////////////////////////////////////////////////
// Name:        scatterplotrendertests.cpp
// Purpose:     Characterization tests for ScatterPlot layout output
// Author:      Blake Madden
// Copyright:   (c) 2026 Blake Madden
// License:     3-Clause BSD license
///////////////////////////////////////////////////////////////////////////////

// These tests lock in the observable layout output of ScatterPlot: left-Y and
...
namespace
    {
    // One drawn snapshot of everything a test can observe about a laid-out scatter plot.
    ...
    // A minimal recipe for a scatter plot under test.
    struct ChartSpec
        {
        ...
        bool m_showRegressionLines{ false };
        };

    [[nodiscard]]
    std::shared_ptr<ScatterPlot> BuildChart(Canvas* canvas, const ChartSpec& spec)
        {
        auto dataset =
        ...
        auto chart = std::make_shared<ScatterPlot>(canvas);
        chart->ShowRegressionLines(spec.m_showRegressionLines);
    ...
    [[nodiscard]]
    LayoutFingerprint LayOutAndCapture(Canvas* canvas, const std::shared_ptr<ScatterPlot>& chart)
        {
    ...
    // The matrix of scatter-plot recipes exercised by several tests below.
    [[nodiscard]]
    std::vector<std::pair<std::string, ChartSpec>> AllSpecs()
        ...
        specs.push_back({ "regression-on",
                          ChartSpec{ .m_showRegressionLines = true } });
        return specs;
        }

    // Exact layout output recorded from the [.dump] test, keyed by spec name.
    // Update only for a deliberate, reviewed change to ScatterPlot layout.
    [[nodiscard]]
...
TEST_CASE("ScatterPlot layout is deterministic and idempotent", "[scatterplot][render]")
    {
...
// to ScatterPlot layout.
TEST_CASE("ScatterPlot layout matches the recorded baseline", "[scatterplot][render]")
    {
    for (const auto& [name, spec] : AllSpecs())
        {
        SECTION(name)
...
TEST_CASE("ScatterPlot axes enclose the data", "[scatterplot][render]")
    {
    for (const auto& [name, spec] : AllSpecs())
        {
        SECTION(name)
...
TEST_CASE("ScatterPlot builds one series per group", "[scatterplot][render]")
    {
    for (const auto& [name, spec] : AllSpecs())
        {
        SECTION(name)
...
TEST_CASE("ScatterPlot characterization dump", "[scatterplot][render][.dump]")
    {
    const wxString outPath{ wxFileName(wxStandardPaths::Get().GetExecutablePath()).GetPath() +
                            L"/scatterplot_characterization.txt" };
