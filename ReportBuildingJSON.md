...
- `"print"` properties related to printer settings.\n
  ...
  - `"paper-size"`: the paper size to print to.
    ...
    - `"paper-env-12"` #12 Envelope, 4 34 by 11 inches.
    ...
    - `"paper-env-personal"` 6 34 Envelope, 3 5.
...
- `"rows"`: an array of rows, each containing items to draw on the page (layed out horizontally).\n
  - `"items"`: an array of items in the current row.
    - `"type"`: the type of object that the item is (e.g., `Wisteria::Graphs::Table`, `Wisteria::Graphs::LinePlot`, etc.)\n
      Available options are:
      ...
      - `null`: a `null` value will act as a placeholder for the previous column.\n
...
- `"resize-method"`: How the image's size is adjusted when its boudning box is changed.\n
  Available options are:
  - `"downscale-only"`
...
- `"showcase-slices"` draw attention to a slice (or series of slices).\n
  Available options are:
...
- `"offset"`: a numeric value combined with the value for `"origin"`.\n
     This is optional and is useful for when `"origin"` is interpreted at runtime.\n
